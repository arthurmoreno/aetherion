...
public:
  ...
  EntityMouseSelection(int screenX, int screenY, EntityInterface &entity,
                       int lockOnTarget, int layerIndex,
                       const std::map<std::string, int> &mouseState,
                       int TILE_SIZE_ON_SCREEN)
      : screenX(screenX), screenY(screenY), entity(entity),
        lockOnTarget(lockOnTarget), layerIndex(layerIndex),
  ...
  bool isMouseCoordinatesInvalid() const {
  ...
  void setSelectionAndDrawHovered(std::shared_ptr<RenderQueue> renderQueuePtr,
                                  int layerIndex,
                                  const std::string &groupToDraw);
  ...
  bool checkVoxelBottomSelection(WorldView &worldView,
                                 ...
                                 int layerIndex,
                                 const std::string &groupToDraw);
  bool checkVoxelTopSelection(WorldView &worldView,
                              std::shared_ptr<RenderQueue> renderQueuePtr,
