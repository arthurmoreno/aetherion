...
public:
  ...
  bool useActiveMask =
      true; // If false, entityGrid is the authoritative activity
  ...
  // [TODO: Use a map to store default values for terrains! -- this is mostly
  // redundant data]
  ...
  StructuralIntegrityComponent getTerrainStructuralIntegrity(int x, int y,
                                                             int z) const;
  ...
  //      GradientVector gradientVector;
  void setTerrainGradientVector(int x, int y, int z,
...
template <typename Callback>
void TerrainStorage::iterateGrid(const openvdb::Int32Grid::Ptr &grid,
                                 Callback callback, int minValue) const {
  if (!grid)
    return;
