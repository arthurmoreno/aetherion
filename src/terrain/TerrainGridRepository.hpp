...
public:
  ...
  // Arbitration helper: inactive ⇒ VDB only; active ⇒ VDB static + ECS
  // transient
  ...
  StructuralIntegrityComponent
  getTerrainStructuralIntegrity(int x, int y, int z,
                                bool takeLock = true) const;
  ...
  void setTerrainMatterContainer(int x, int y, int z,
  ...
  // destroying it. This removes transient components (Velocity/MovingComponent)
  ...
  template <typename Callback> void iterateWaterMatter(Callback callback) const;

  template <typename Callback> void iterateVaporMatter(Callback callback) const;
  ...
  // - `respectTerrainGridLock`: when true (default) the helper will skip
  // acquiring
  ...
  auto
  withTrackingMapsLock(Func &&func, bool takeTrackingLock = true,
...
private:
  ...
  auto withSharedLock(Func &&func,
                      bool takeLock = true) const -> decltype(func()) {
    if (!takeLock) {
      return func();
    }
...
void TerrainGridRepository::iterateWaterMatter(Callback callback) const {
  storage_.iterateWaterMatter(
      [this, callback](int x, int y, int z, int amount) {
        // Provide both static water amount and full terrain info (including
        // transients)
...
void TerrainGridRepository::iterateVaporMatter(Callback callback) const {
  storage_.iterateVaporMatter(
      [this, callback](int x, int y, int z, int amount) {
        // Provide both static vapor amount and full terrain info (including
        // transients)
        TerrainInfo info = this->readTerrainInfo(x, y, z);
        callback(x, y, z, amount, info);
      });
...
void TerrainGridRepository::iterateBiomassMatter(Callback callback) const {
  storage_.iterateBiomassMatter(
      [this, callback](int x, int y, int z, int amount) {
        // Provide both static biomass amount and full terrain info (including
        // transients)
        TerrainInfo info = this->readTerrainInfo(x, y, z);
        callback(x, y, z, amount, info);
      });
