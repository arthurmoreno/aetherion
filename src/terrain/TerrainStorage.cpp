...
inline GradientVector unpackGradFromBits(uint32_t bits) {
  ...
  return GradientVector{dequantizeGrad(sgx), dequantizeGrad(sgy),
                        dequantizeGrad(sgz)};
}

inline uint32_t setBits(uint32_t flags, int shift, uint32_t mask,
                        uint32_t value) {
  flags &= ~(mask << shift);
...
// Helper functions to decode from flags directly without additional tree
// lookups
inline bool decodeCanStackEntities(uint32_t flags) {
  return getBits(flags, FlagBits::CANSTACK_SHIFT, FlagBits::CANSTACK_MASK) !=
         0u;
}

inline GradientVector decodeGradientVector(uint32_t flags) {
  uint32_t gradBits =
      getBits(flags, FlagBits::GRADIENT_SHIFT, FlagBits::GRADIENT_MASK);
  return unpackGradFromBits(gradBits);
}

inline DirectionEnum decodeDirection(uint32_t flags) {
  uint32_t dirVal =
      getBits(flags, FlagBits::DIRECTION_SHIFT, FlagBits::DIRECTION_MASK);
  if (dirVal == 0)
    return DirectionEnum::UP; // default
  return static_cast<DirectionEnum>(static_cast<int>(dirVal));
}

inline MatterState decodeMatterState(uint32_t flags) {
  uint32_t val =
      getBits(flags, FlagBits::MATTERSTATE_SHIFT, FlagBits::MATTERSTATE_MASK);
  return static_cast<MatterState>(static_cast<int>(val) + 1);
}

// Helper function to encode StructuralIntegrityComponent into flags
inline uint32_t
encodeStructuralIntegrity(const StructuralIntegrityComponent &sic,
  ...
  if (matterVal > 3)
    matterVal = 3;
  flags = setBits(flags, FlagBits::MATTERSTATE_SHIFT,
                  FlagBits::MATTERSTATE_MASK, static_cast<uint32_t>(matterVal));
...
inline uint32_t encodeDirection(uint32_t flags, DirectionEnum direction) {
  ...
  return setBits(flags, FlagBits::DIRECTION_SHIFT, FlagBits::DIRECTION_MASK,
                 dirVal);
}

inline uint32_t encodeCanStackEntities(uint32_t flags, bool canStack) {
  return setBits(flags, FlagBits::CANSTACK_SHIFT, FlagBits::CANSTACK_MASK,
                 canStack ? 1u : 0u);
}
...
inline uint32_t encodeGradientVector(uint32_t flags,
...
TerrainStorage::TerrainStorage() {
  ...
  // (e.g., in tests that don't create a VoxelGrid, which normally initializes
  // OpenVDB).
  openvdb::initialize();
...
void TerrainStorage::applyTransform(double voxelSize_) {
  ...
  if (terrainGrid)
    terrainGrid->setTransform(xform);
  if (mainTypeGrid)
    mainTypeGrid->setTransform(xform);
...
void TerrainStorage::configureThreadCache() {
  ...
  const void *tptr =
      terrainGrid ? static_cast<const void *>(&terrainGrid->tree()) : nullptr;
  ...
  if (!tc.configured || changed) {
    ...
    tc.subType0Acc = std::make_unique<openvdb::Int32Grid::Accessor>(
        subType0Grid->getAccessor());
    tc.subType1Acc = std::make_unique<openvdb::Int32Grid::Accessor>(
        subType1Grid->getAccessor());
    tc.terrainMatterAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        terrainMatterGrid->getAccessor());
    tc.waterMatterAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        waterMatterGrid->getAccessor());
    tc.vaporMatterAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        vaporMatterGrid->getAccessor());
    tc.biomassMatterAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        biomassMatterGrid->getAccessor());
    tc.massAcc =
        std::make_unique<openvdb::Int32Grid::Accessor>(massGrid->getAccessor());
    tc.maxSpeedAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        maxSpeedGrid->getAccessor());
    tc.minSpeedAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        minSpeedGrid->getAccessor());
    tc.flagsAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        flagsGrid->getAccessor());
    tc.maxLoadCapacityAcc = std::make_unique<openvdb::Int32Grid::Accessor>(
        maxLoadCapacityGrid->getAccessor());
    if (heatGrid)
      tc.heatAcc = std::make_unique<openvdb::FloatGrid::Accessor>(
...
int64_t TerrainStorage::getTerrainIdIfExists(int x, int y, int z) {
  ...
  if (s_threadCache.terrainAcc) {
    ...
    if (entityId != -2) {
      // std::cout << "Found entity ID on terrainGrid: " << entityId <<
      // std::endl;

      // if (entityId != -1) {
      //     std::cout << "[getTerrainIdIfExists] Checkpoint! (" << x << ", " <<
      //     y << ", " << z << ")\n";
      // }
      return entityId;
    }
...
void TerrainStorage::setTerrainCanStackEntities(int x, int y, int z,
                                                bool canStack) {
  auto c = openvdb::Coord(x, y, z);
  uint32_t flags = static_cast<uint32_t>(flagsGrid->tree().getValue(c));
...
void TerrainStorage::setTerrainMatterState(int x, int y, int z,
                                           MatterState state) {
  auto c = openvdb::Coord(x, y, z);
  uint32_t flags = static_cast<uint32_t>(flagsGrid->tree().getValue(c));
...
void TerrainStorage::setTerrainGradientVector(int x, int y, int z,
                                              const GradientVector &gradient) {
  auto c = openvdb::Coord(x, y, z);
  uint32_t flags = static_cast<uint32_t>(flagsGrid->tree().getValue(c));
...
GradientVector TerrainStorage::getTerrainGradientVector(int x, int y,
                                                        int z) const {
  auto c = openvdb::Coord(x, y, z);
  uint32_t flags = static_cast<uint32_t>(flagsGrid->tree().getValue(c));
...
void TerrainStorage::setTerrainMaxLoadCapacity(int x, int y, int z,
                                               int capacity) {
...
bool TerrainStorage::checkIfTerrainExists(int x, int y, int z) const {
  ...
  return terrainType !=
         -2; // -2 = off nodes (no terrain), -1/-0/+ = terrain exists
}
...
void TerrainStorage::setTerrainStructuralIntegrity(
  ...
  flagsGrid->tree().setValue(coord,
                             encodeStructuralIntegrity(sic, encodedFlags));
  // maxLoadCapacity is stored in its own grid (matching
  // getTerrainStructuralIntegrity)
  maxLoadCapacityGrid->tree().setValue(coord, sic.maxLoadCapacity);
}

StructuralIntegrityComponent
TerrainStorage::getTerrainStructuralIntegrity(int x, int y, int z) const {
  openvdb::Coord coord(x, y, z);
  uint32_t encodedFlags =
      static_cast<uint32_t>(flagsGrid->tree().getValue(coord));
