...
public:
  ...
  PyRegistry(entt::registry &reg, entt::dispatcher &disp)
  ...
  nb::list view(const std::vector<std::string> &component_names) {
    ...
    static const std::unordered_map<std::string, entt::id_type>
        component_type_map = {
            ...
            {"MetabolismComponent",
             entt::type_hash<MetabolismComponent>::value()},
            {"DigestionComponent",
             entt::type_hash<DigestionComponent>::value()},
            {"FoodItem", entt::type_hash<FoodItem>::value()},
            {"EntityTypeComponent",
             entt::type_hash<EntityTypeComponent>::value()},
            {"WeaponAttributes", entt::type_hash<WeaponAttributes>::value()},
            {"Durability", entt::type_hash<Durability>::value()},
            {"MeeleAttackComponent",
             entt::type_hash<MeeleAttackComponent>::value()},
            {"ConsoleLogsComponent",
             entt::type_hash<ConsoleLogsComponent>::value()}};

    // Create a runtime_view
    entt::runtime_view view;

    // Iterate over requested components and add their storages to the
    // runtime_view
    for (const auto &name : component_names) {
      if (auto it = component_type_map.find(name);
          it != component_type_map.end()) {
  ...
  template <typename Component> bool all_of_single(entt::entity entity) {
    return registry.all_of<Component>(entity);
  }

  bool has_all_components(uint32_t entity_id,
  ...
  // Get a component from an entity
  nb::object get_component(uint32_t entity_id,
...
private:
  ...
  template <typename View> nb::list entities_to_pylist(View view) {
    nb::list entity_list;
    for (auto entity : view) {
