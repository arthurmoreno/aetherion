...
VoxelGrid::VoxelGrid(entt::registry &reg) : registry(reg) {
  ...
  terrainGridRepository =
      std::make_unique<TerrainGridRepository>(registry, *terrainStorage);

  // Create empty grids for entity, event, and lighting (terrain now managed by
  // TerrainGridRepository)
  entityGrid =
      openvdb::Int32Grid::create(defaultEmptyValue); // Default entity ID = 0
  eventGrid =
      openvdb::Int32Grid::create(defaultEmptyValue); // Default event ID = 0
  lightingGrid =
      openvdb::FloatGrid::create(0.0f); // Default lighting level = 0.0
}
...
void VoxelGrid::initializeGrids() {
  // Terrain grid is now managed by TerrainGridRepository, so we only initialize
  // the others
  entityGrid->setTransform(
      openvdb::math::Transform::createLinearTransform(1.0));
  eventGrid->setTransform(openvdb::math::Transform::createLinearTransform(1.0));
  lightingGrid->setTransform(
      openvdb::math::Transform::createLinearTransform(1.0));
...
void VoxelGrid::setVoxel(int x, int y, int z, const GridData &data) {
  // Set terrain using TerrainGridRepository (using mainType for now as
  // terrainID)
  ...
  // PROTECTED: Set entity data with mutex protection
  {
    ...
    if (entityGrid) {
      entityGrid->getAccessor().setValue(openvdb::Coord(x, y, z),
                                         data.entityID);
    }
  }
  ...
  lightingGrid->getAccessor().setValue(openvdb::Coord(x, y, z),
                                       data.lightingLevel);
}
...
GridData VoxelGrid::getVoxel(int x, int y, int z) const {
  ...
  // PROTECTED: Read entity data with mutex protection
  {
    ...
    if (entityGrid) {
      data.entityID =
          entityGrid->getConstAccessor().getValue(openvdb::Coord(x, y, z));
    } else {
      data.entityID = defaultEmptyValue;
    }
  }
  data.eventID =
      eventGrid->getConstAccessor().getValue(openvdb::Coord(x, y, z));
  data.lightingLevel =
      lightingGrid->getConstAccessor().getValue(openvdb::Coord(x, y, z));
...
void VoxelGrid::setTerrain(int x, int y, int z, int terrainID) {
  if (terrainGridRepository && terrainID != -1 && terrainID != -2) {
    // Instead of directly setting mainType, we need to check if there's an
    // entity at this location that needs to be migrated to OpenVDB storage
    ...
    throw std::runtime_error(
        "TerrainGridRepository not initialized or invalid terrainID");
  }
}
...
int64_t VoxelGrid::getTerrain(int x, int y, int z) const {
  if (terrainGridRepository) {
    ...
    if (terrainId && *terrainId != -2) {
      // if (*terrainId == -1) {
      //     // std::cout << "VoxelGrid::getTerrain: Terrain exists but no enTT
      //     entity at ("
      //     //           << x << ", " << y << ", " << z << ")\n";
      // } else {
      //     // std::cout << "VoxelGrid::getTerrain: Found terrain ID " <<
      //     *terrainId
      //     //           << " at (" << x << ", " << y << ", " << z << ")\n";
      // }
      return *terrainId;
...
EntityTypeComponent VoxelGrid::getTerrainEntityTypeComponent(int x, int y,
                                                             int z) const {
  if (terrainGridRepository) {
    return terrainGridRepository->getTerrainEntityType(x, y, z);
  }
  return EntityTypeComponent(); // Return a default-constructed
                                // EntityTypeComponent if not found
}

bool VoxelGrid::setTerrainEntityTypeComponent(
    int x, int y, int z, EntityTypeComponent component) const {
  if (terrainGridRepository) {
...
int VoxelGrid::getEntity(int x, int y, int z) const {
  ...
  // Performance optimization: check if voxel is active first to avoid tree
  // traversal
...
void VoxelGrid::deleteEntity(int x, int y, int z) {
  ...
  accessor.setValueOff(coord,
                       defaultEmptyValue); // Properly deactivate node for
                                           // OpenVDB tree cleanliness
}
...
std::vector<char> VoxelGrid::serializeToBytes() const {
  ...
  // Populate the map with voxel data using TerrainStorage's mainTypeGrid
  // iterator
  if (terrainStorage && terrainStorage->mainTypeGrid) {
    std::shared_lock<std::shared_mutex> lock(
        entityGridMutex); // PROTECT entity grid access

    for (auto iter = terrainStorage->mainTypeGrid->cbeginValueOn(); iter.test();
         ++iter) {
      ...
      GridData data;
      data.terrainID =
          terrainStorage->getTerrainMainType(coord.x(), coord.y(), coord.z());

      // PROTECTED: Read entity data with mutex protection
      if (entityGrid) {
        data.entityID =
            entityGrid->tree().getValue(coord); // Line 206 - NOW PROTECTED
      } else {
        data.entityID = defaultEmptyValue;
      }
...
void VoxelGrid::deserializeFromBytes(const std::vector<char> &byteData) {
  ...
  // Populate the grids using the data from the map
  {
    ...
    for (const auto &[coordinates, data] : voxelDataMap) {
      ...
      if (terrainGridRepository) {
        terrainGridRepository->setTerrainMainType(
            coordinates.x, coordinates.y, coordinates.z, data.terrainID);
      }

      // PROTECTED: Set entity data with mutex protection
      if (entityGrid && data.entityID != defaultEmptyValue) {
        entityGrid->tree().setValue(coord,
                                    data.entityID); // Line 248 - NOW PROTECTED
      }

      // Set other grid data (outside the entity mutex - these can be moved
      // outside if needed)
...
template <typename Packer> void VoxelGrid::msgpack_pack(Packer &pk) const {
...
void VoxelGrid::msgpack_unpack(msgpack::object const &o) {
  ...
  if (o.type != msgpack::type::BIN) {
    throw std::runtime_error(
        "Expected binary data for VoxelGrid deserialization");
  }
...
std::vector<VoxelGridCoordinates>
VoxelGrid::getAllTerrainInRegion(int x_min, int y_min, int z_min, int x_max,
  ...
  if (terrainStorage && terrainStorage->mainTypeGrid) {
    ...
    // Use OpenVDB's efficient spatial iteration - only traverses voxels in the
    // bbox
    auto accessor = terrainStorage->mainTypeGrid->getConstAccessor();

    // Iterate only through coordinates in the bounding box
    for (openvdb::CoordBBox::Iterator coordIter = bbox.begin(); coordIter;
         ++coordIter) {
      const openvdb::Coord &coord = *coordIter;

      // Check if this coordinate has an active voxel (much faster than full
      // iteration)
      if (accessor.isValueOn(coord)) {
        result.emplace_back(
            VoxelGridCoordinates{coord.x(), coord.y(), coord.z()});
      }
    }
  }
...
std::vector<VoxelGridCoordinates>
VoxelGrid::getAllEntityInRegion(int x_min, int y_min, int z_min, int x_max,
                                int y_max, int z_max) const {
  // Read operations use shared lock (multiple readers allowed)
  std::shared_lock<std::shared_mutex> lock(entityGridMutex);
...
std::vector<VoxelGridCoordinates>
VoxelGrid::getAllEventInRegion(int x_min, int y_min, int z_min, int x_max,
                               int y_max, int z_max) const {
  std::vector<VoxelGridCoordinates> result;
...
std::vector<VoxelGridCoordinates>
VoxelGrid::getAllLightingInRegion(int x_min, int y_min, int z_min, int x_max,
                                  int y_max, int z_max) const {
  std::vector<VoxelGridCoordinates> result;
...
void VoxelGridView::initVoxelGridView(int width, int height, int depth,
...
void VoxelGridView::setTerrainVoxel(int x, int y, int z, const int voxelData) {
  ...
  if (local_x >= 0 && local_x < width && local_y >= 0 && local_y < height &&
      local_z >= 0 && local_z < depth) {
    terrainData[local_x + local_y * width + local_z * width * height] =
        voxelData;
  } else {
    // Handle out-of-bounds access
    std::cerr << "Attempted to set voxel out of bounds at (" << x << ", " << y
              << ", " << z << ")" << std::endl;
  }
}
...
void VoxelGridView::setEntityVoxel(int x, int y, int z, const int voxelData) {
  ...
  if (local_x >= 0 && local_x < width && local_y >= 0 && local_y < height &&
      local_z >= 0 && local_z < depth) {
    entityData[local_x + local_y * width + local_z * width * height] =
        voxelData;
  } else {
    // Handle out-of-bounds access
    std::cerr << "Attempted to set voxel out of bounds at (" << x << ", " << y
...
std::vector<int>
VoxelGrid::getAllTerrainIdsInRegion(int x_min, int y_min, int z_min, int x_max,
                                    int y_max, int z_max,
  ...
  openvdb::Int32Grid::ConstAccessor accessor =
      terrainStorage->mainTypeGrid->getConstAccessor();
...
std::vector<int>
VoxelGrid::getAllEntityIdsInRegion(int x_min, int y_min, int z_min, int x_max,
                                   int y_max, int z_max,
                                   VoxelGridView &gridView) const {
  // Read operations use shared lock (multiple readers allowed)
  std::shared_lock<std::shared_mutex> lock(entityGridMutex);
...
std::vector<int> VoxelGrid::getAllEventIdsInRegion(int x_min, int y_min,
                                                   int z_min, int x_max,
                                                   int y_max, int z_max) const {
  std::vector<int> result;
...
std::vector<int> VoxelGrid::getAllLightingIdsInRegion(int x_min, int y_min,
                                                      int z_min, int x_max,
                                                      int y_max,
                                                      int z_max) const {
  std::vector<int> result;
...
void VoxelGrid::moveEntity(entt::entity entity, Position movingToPosition) {
  ...
  if (auto posPtr = registry.try_get<Position>(entity)) {
    ...
    // Use unsafe method since we already hold the lock - avoids recursive
    // locking
    ...
    if (entityId == entityVoxelGridId) {
      ...
      openvdb::Coord newCoord(movingToPosition.x, movingToPosition.y,
                              movingToPosition.z);
    ...
    std::cout << "Warning: moveEntity called for entity without Position: id="
              << entityId << std::endl;
  }
}
