...
void setToDrawSelectEntitySquare(int x, int y,
  ...
  renderQueuePtr->add_task_draw_rect(layerIndex, groupToDraw, x, y,
                                     selectedAreaWidth, selectedAreaHeight, 3,
                                     selectedSDLColor);
}

void EntityMouseSelection::drawLockOnTarget(
    std::shared_ptr<RenderQueue> renderQueuePtr,
    const std::string &groupToDraw) {
  std::cout << "[drawLockOnTarget] Value of lockOnTarget = " << lockOnTarget
            << std::endl;
  std::cout << "[drawLockOnTarget] Value of entity.getEntityId() = "
  ...
  if (lockOnTarget != -1 && entity.getEntityId() == lockOnTarget) {
    std::cout << "[drawLockOnTarget] Inside the if offlockOnTarget != -1 && "
              ...
              << std::endl;
    setToDrawSelectEntitySquare(offsetX, offsetY, renderQueuePtr, layerIndex,
                                groupToDraw, true, TILE_SIZE_ON_SCREEN);

    std::ostringstream errorMessage;
    errorMessage << "Have caught an entity. EntityID: " << entity.getEntityId()
                 << std::endl;
...
void EntityMouseSelection::setSelectionAndDrawHovered(
    std::shared_ptr<RenderQueue> renderQueuePtr, int layerIndex,
    const std::string &groupToDraw) {
  currentEntitySelected = true;
  if (!selectedSquareDrawn) {
    setToDrawSelectEntitySquare(offsetX, offsetY, renderQueuePtr, layerIndex,
                                groupToDraw, false, TILE_SIZE_ON_SCREEN);
  }
}

bool EntityMouseSelection::checkVoxelBottomSelection(
    WorldView &worldView, std::shared_ptr<RenderQueue> renderQueuePtr,
    int layerIndex, const std::string &groupToDraw) {
  std::cout << "Inside the checkVoxelBottomSelection()." << std::endl;
  if (isMouseWithin(mouseX, mouseY, offsetX, offsetY, TILE_SIZE_ON_SCREEN,
                    TILE_SIZE_ON_SCREEN)) {
    std::cout << "Inside the if  checkVoxelBottomSelection() and isMouseWithin."
              << std::endl;
    auto cornerSETerrain =
        worldView.checkIfTerrainExist(entityX + 1, entityY + 1, entityZ);
    auto aboveSETerrain =
        worldView.checkIfTerrainExist(entityX + 2, entityY + 2, entityZ + 1);
...
bool EntityMouseSelection::checkVoxelTopSelection(
    WorldView &worldView, std::shared_ptr<RenderQueue> renderQueuePtr,
    int layerIndex, const std::string &groupToDraw) {
  ...
  if (entityType.mainType == static_cast<int>(EntityEnum::TERRAIN)) {
    ...
    std::cout << "Inside the checkVoxelTopSelection(); variables values: "
              << "mouseX: " << mouseX << "; mouseY: " << mouseY
              << "; screenX: " << screenX << "; screenY: " << screenY
              << "; TILE_SIZE_ON_SCREEN: " << TILE_SIZE_ON_SCREEN << std::endl;
    if (isMouseWithin(mouseX, mouseY, screenX, screenY, TILE_SIZE_ON_SCREEN,
                      TILE_SIZE_ON_SCREEN)) {
      std::cout << "Inside the if  checkVoxelTopSelection() and isMouseWithin."
                << std::endl;
      auto aboveSETerrainExist =
          worldView.checkIfTerrainExist(entityX + 1, entityY + 1, entityZ + 1);
      auto aboveEntityExist =
          worldView.checkIfEntityExist(entityX, entityY, entityZ + 1);
...
//     world_view, entity, mouse_state, screen_x, screen_y, render_queue,
//     layer_index, camera, selected_entity
// ):
//     entity_mouse_selection = EntityMouseSelection(screen_x, screen_y, entity,
//     selected_entity, layer_index, mouse_state)
//     entity_mouse_selection.draw_lock_on_target(render_queue, camera)
...
//     if entity_mouse_selection.check_voxel_botton_selection(world_view,
//     render_queue, layer_index, camera):
//         return entity_mouse_selection.current_entity_selected

//     if entity_mouse_selection.check_voxel_top_selection(world_view,
//     render_queue, layer_index, camera):
//         return entity_mouse_selection.current_entity_selected

// TODO: Organize better the order of parameters here
bool getAndDrawSelectedEntity(WorldView &world_view,
                              EntityInterface &entity_interface,
                              nb::dict &mouse_state, int screenX, int screenY,
                              std::shared_ptr<RenderQueue> render_queue_ptr,
                              int layer_index, int selected_entity_id,
  ...
  if (entity_mouse_selection.isMouseCoordinatesInvalid()) {
    std::cout << "Inside the if  off "
                 "entity_mouse_selection.isMouseCoordinatesInvalid()."
              << std::endl;
    return entity_mouse_selection.currentEntitySelected;
  }
  entity_mouse_selection.setEntityCoordinates();

  if (entity_mouse_selection.checkVoxelBottomSelection(
          world_view, render_queue_ptr, layer_index, groupToDraw)) {
    std::cout << "Inside the if  checkVoxelBottomSelection()." << std::endl;
    return entity_mouse_selection.currentEntitySelected;
  }

  if (entity_mouse_selection.checkVoxelTopSelection(
          world_view, render_queue_ptr, layer_index, groupToDraw)) {
...
//     return (
//         terrain.get_entity_type().sub_type0 == lifesimcore.TerrainEnum_EMPTY
//         and EMPTY_TILE_DEBUGGING
...
//         and terrain.get_entity_type().sub_type0 ==
//         lifesimcore.TerrainEnum_WATER and
//         terrain.get_matter_container().water_matter == 0 and
//         terrain.get_matter_container().water_vapor == 0
//     )
bool isTerrainAnEmptyWater(const EntityInterface &terrain) {
  if (terrain.hasComponent(ComponentFlag::ENTITY_TYPE) &&
    ...
    const MatterContainer matterContainer =
        terrain.getComponent<MatterContainer>();
...
bool isOccludingEntityPerspective(const EntityInterface &entity,
...
bool isOccludingSomeEntity(const WorldView &worldView,
                           const EntityInterface &occludingEntity) {
  // Get the position of the occluding entity
  const Position occludingEntityPos = occludingEntity.getComponent<Position>();
  const int occludingEntityX = occludingEntityPos.x;
  ...
  if (occludingEntityType.mainType == static_cast<int>(EntityEnum::TERRAIN)) {
    // Check the 3 critical voxel positions for terrain occlusion at same Z
    // level:
    // 1. Direct overlap position
    if (worldView.checkIfEntityExist(occludingEntityX, occludingEntityY,
                                     occludingEntityZ)) {
      return true;
    }
