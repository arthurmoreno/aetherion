...
nb::object variant_to_pyobject(const ItemConfiguration::DefaultValue &var) {
  return std::visit([](auto &&arg) -> nb::object { return nb::cast(arg); },
                    var);
}
...
NB_MODULE(_aetherion, m) {
  ...
  nb::bind_map<std::map<std::string, std::map<std::string, std::string>>>(
      m, "MapStrMapStrStr");
  nb::bind_map<std::map<std::string, std::map<std::string, double>>>(
      m, "MapStrMapStrDouble");
  ...
  nb::class_<Command>(m, "Command")
      .def(nb::init<std::string>(), nb::arg("type"),
           "Create a command with a type")
      .def(nb::init<>(), "Create an empty command")
      .def("get_type", &Command::getType, "Get the command type")
      .def("set_type", &Command::setType, nb::arg("type"),
           "Set the command type")
      .def("has_param", &Command::hasParam, nb::arg("key"),
      ...
      .def("to_string", &Command::toString,
           "Get string representation for debugging")
      .def_prop_rw("type", &Command::getType, &Command::setType,
                   "Command type property")
      .def("__repr__", &Command::toString)
      // Dict-like access for Python compatibility (delegates to nb::dict
      // params_)
      .def(
          "__getitem__",
      ...
      .def(
          ...
          "Set parameter value (dict-like access)")
      .def(
          "__contains__",
  ...
  nb::class_<DropToWorldCommand, Command>(m, "DropToWorldCommand")
      ...
      .def("get_item_index", &DropToWorldCommand::getItemIndex,
           "Get the item index")
      .def("get_src_window", &DropToWorldCommand::getSrcWindow,
           "Get the source window")
      .def("get_world_x", &DropToWorldCommand::getWorldX,
           "Get the world X coordinate")
      .def("get_world_y", &DropToWorldCommand::getWorldY,
  ...
  nb::class_<EditorCommand, Command>(m, "EditorCommand")
      ...
      .def_static("action_to_string", &EditorCommand::actionToString,
                  nb::arg("action"), "Convert editor action to string");
  ...
  m.def(
      ...
      [](int worldTicks, float availableFps,
        ...
        renderInGameGuiFrame(worldTicks, availableFps, world_ptr,
                             physicsChanges, inventoryData, consoleLogs,
                             entitiesData, commands, statistics, shared_data,
      ...
      nb::arg("physicsChanges"), nb::arg("inventoryData"),
      nb::arg("consoleLogs"), nb::arg("entitiesData"), nb::arg("commands"),
      nb::arg("statistics"), nb::arg("shared_data"),
      nb::arg("entityInterface_ptr"), nb::arg("hoveredEntityInterface_ptr"),
      nb::arg("selectedEntityInterface_ptr"));
  m.def("imgui_prepare_editor_windows", &imguiPrepareEditorWindows,
        nb::arg("commands"), nb::arg("shared_data"), nb::arg("voxel_data"),
        "Prepare the editor windows for ImGui rendering");
  m.def("imgui_prepare_title_windows", &imguiPrepareTitleWindows,
        nb::arg("commands"), nb::arg("shared_data"),
        "Prepare the title windows for ImGui rendering");
  m.def("imgui_prepare_world_type_form_windows",
        &imguiPrepareWorldTypeFormWindows, nb::arg("commands"),
        nb::arg("shared_data"),
        "Prepare the world type selection form windows for ImGui rendering");
  m.def("imgui_prepare_server_world_form_windows",
        &imguiPrepareServerWorldFormWindows, nb::arg("commands"),
        nb::arg("shared_data"),
        "Prepare the server world form windows for ImGui rendering");
  m.def("imgui_prepare_world_form_windows", &imguiPrepareWorldFormWindows,
        nb::arg("commands"), nb::arg("shared_data"),
        "Prepare the world creation form windows for ImGui rendering");
  m.def("imgui_prepare_world_list_windows", &imguiPrepareWorldListWindows,
        nb::arg("commands"), nb::arg("shared_data"),
        "Prepare the world list selection windows for ImGui rendering");
  m.def("imgui_prepare_character_form_windows",
        &imguiPrepareCharacterFormWindows, nb::arg("commands"),
        nb::arg("shared_data"),
        "Prepare the character creation form windows for ImGui rendering");
  m.def("imgui_prepare_character_list_windows",
        &imguiPrepareCharacterListWindows, nb::arg("commands"),
        nb::arg("shared_data"),
  ...
  m.def("render_texture", &render_texture, "Render a texture",
        nb::arg("renderer_ptr"), nb::arg("texture_ptr"), nb::arg("x"),
        nb::arg("y"));
  m.def("destroy_texture", &destroy_texture, "Destroy a texture",
        nb::arg("texture_ptr"));
  ...
  m.def("load_texture_gl", &load_texture_gl,
        "Load an image and create an OpenGL texture", nb::arg("gl_context_ptr"),
        nb::arg("image_path"));
  m.def("render_texture_gl", &render_texture_gl, "Render an OpenGL texture",
        nb::arg("gl_context_ptr"), nb::arg("texture_id"), nb::arg("x"),
        nb::arg("y"), nb::arg("width") = -1, nb::arg("height") = -1);
  m.def("destroy_texture_gl", &destroy_texture_gl, "Destroy an OpenGL texture",
        nb::arg("texture_id"));

  // OpenGL texture manager functions
  m.def("load_texture_on_manager_gl", &loadTextureOnManagerGL,
        "Load texture into OpenGL manager", nb::arg("gl_context_ptr"),
        nb::arg("imagePath"), nb::arg("id"), nb::arg("newWidth") = -1,
        nb::arg("newHeight") = -1);
  m.def("render_texture_from_manager_gl", &renderTextureFromManagerGL,
        "Render texture from OpenGL manager", nb::arg("gl_context_ptr"),
        nb::arg("id"), nb::arg("x"), nb::arg("y"), nb::arg("width") = -1,
        nb::arg("height") = -1);
  ...
  m.def("wants_capture_keyboard", &wants_capture_keyboard,
        "Returns True if ImGui wants to capture keyboard inputs, False "
        "otherwise");
  ...
  nb::class_<TerrainStorage>(m, "TerrainStorage")
      ...
      .def("set_terrain_biomass_matter",
           &TerrainStorage::setTerrainBiomassMatter)
      .def("get_terrain_biomass_matter",
           &TerrainStorage::getTerrainBiomassMatter)
      // Physics
      ...
      .def("set_terrain_max_load_capacity",
           &TerrainStorage::setTerrainMaxLoadCapacity)
      .def("get_terrain_max_load_capacity",
           &TerrainStorage::getTerrainMaxLoadCapacity)
  ...
  nb::class_<TerrainGridRepository>(m, "TerrainGridRepository")
      .def(nb::init<entt::registry &, TerrainStorage &>(),
           nb::keep_alive<1, 2>(), // Keep registry (arg 2) alive while self
                                   // (arg 1) is alive
           nb::keep_alive<1, 3>()) // Keep storage (arg 3) alive while self (arg
                                   // 1) is alive
      .def("read_terrain_info", &TerrainGridRepository::readTerrainInfo)
      .def("get_terrain_entity_type",
           &TerrainGridRepository::getTerrainEntityType)
      .def("set_terrain_entity_type",
           &TerrainGridRepository::setTerrainEntityType)
      .def("get_terrain_matter_container",
           &TerrainGridRepository::getTerrainMatterContainer)
      .def("set_terrain_matter_container",
           &TerrainGridRepository::setTerrainMatterContainer)
  ...
  nb::class_<entt::entity>(m, "Entity")
      ...
      .def(
          "__eq__",
          [](const entt::entity &a, const entt::entity &b) { return a == b; },
          "Equality comparison for Entity objects")
      .def(
          "get_id",
          [](const entt::entity &e) { return static_cast<uint32_t>(e); },
          "Get the raw ID of the entity")
      .def(
          "set_id",
          [](entt::entity &e, int new_id) {
            e = static_cast<entt::entity>(new_id);
          },
  ...
  nb::class_<SceneGraph>(m, "SceneGraph")
      ...
      .def(nb::init<entt::registry *>(), nb::arg("registry") = nullptr,
           "Create a SceneGraph. If registry is None, creates its own internal "
           "registry")
      ...
      .def("destroy_node_only", &SceneGraph::destroy_node_only,
           nb::arg("entity"),
           "Destroy only the entity, children are adopted by parent")

      // Hierarchy management
      .def("attach_child", &SceneGraph::attach_child,
           "Attach child under parent. If before is provided, insert before "
           "that sibling, "
           "otherwise append at end")
      .def("attach_node_python_instance",
           &SceneGraph::attach_node_python_instance,
      ...
      .def("get_all_entities_render_order",
           &SceneGraph::get_all_entities_render_order,
  ...
  nb::class_<RenderQueue>(m, "RenderQueue")
      ...
      .def("add_task_by_id_partial", &RenderQueue::add_task_by_id_partial,
           nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("texture_id"),
           nb::arg("x"), nb::arg("y"), nb::arg("lightIntensity"),
           nb::arg("opacity"), nb::arg("src_x"), nb::arg("src_y"),
           nb::arg("src_w"), nb::arg("src_h"),
           "Add a RenderTextureTask with partial texture rendering by texture "
           "ID")
      .def("add_task_by_texture_partial",
           &RenderQueue::add_task_by_texture_partial, nb::arg("z_layer"),
           nb::arg("priority_group"), nb::arg("texture_ptr"), nb::arg("x"),
           nb::arg("y"), nb::arg("lightIntensity"), nb::arg("opacity"),
           nb::arg("src_x"), nb::arg("src_y"), nb::arg("src_w"),
           nb::arg("src_h"),
           "Add a RenderTextureTask with partial texture rendering by texture "
           "pointer")
      ...
      .def(
          ...
          [](RenderQueue &self, int z_layer, const std::string &priority_group,
             ...
             float opacity, int quadrant) {
            RenderQueue::TextureQuadrant quad =
                static_cast<RenderQueue::TextureQuadrant>(quadrant);
            self.add_task_by_id_quadrant(z_layer, priority_group, texture_id, x,
                                         y, lightIntensity, opacity, quad);
          },
          nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("texture_id"),
          nb::arg("x"), nb::arg("y"), nb::arg("lightIntensity"),
          nb::arg("opacity"), nb::arg("quadrant"),
          "Add a RenderTextureTask for a specific quadrant (0=TOP_LEFT, "
          "1=TOP_RIGHT, "
          "2=BOTTOM_LEFT, 3=BOTTOM_RIGHT)")
      .def("add_task_by_id_fraction", &RenderQueue::add_task_by_id_fraction,
           nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("texture_id"),
           nb::arg("x"), nb::arg("y"), nb::arg("lightIntensity"),
           nb::arg("opacity"), nb::arg("x_start_ratio"),
           nb::arg("y_start_ratio"), nb::arg("width_ratio"),
           nb::arg("height_ratio"),
           "Add a RenderTextureTask for a fractional portion of texture "
           "(ratios 0.0-1.0)")
      // Updated add_task_rect binding to accept a tuple
      .def(
          "add_task_rect",
          [](RenderQueue &self, int z_layer, const std::string &priority_group,
             int x, int y, int width, int height, nb::tuple color) {
            ...
            self.add_task_rect(z_layer, priority_group, x, y, width, height,
                               sdl_color);
          },
          nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("x"),
          nb::arg("y"), nb::arg("width"), nb::arg("height"), nb::arg("color"),
      ...
      .def(
          ...
          [](RenderQueue &self, int z_layer, const std::string &priority_group,
            ...
            self.add_task_draw_rect(z_layer, priority_group, x, y, width,
                                    height, thickness, sdl_color);
          },
          nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("x"),
          nb::arg("y"), nb::arg("width"), nb::arg("height"),
          nb::arg("thickness"), nb::arg("color"),
          "Add a RenderRectTask with color as a tuple (R, G, B[, A])")
      ...
      .def(
          "add_task_line",
          [](RenderQueue &self, int z_layer, const std::string &priority_group,
             int x1, int y1, int x2, int y2, nb::tuple color) {
            // Default alpha to 255 (opaque) if not provided
            SDL_Color sdl_color = {0, 0, 0, 255};
            ...
            self.add_task_line(z_layer, priority_group, x1, y1, x2, y2,
                               sdl_color);
          },
          nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("x1"),
          nb::arg("y1"), nb::arg("x2"), nb::arg("y2"), nb::arg("color"),
      ...
      .def(
          ...
          [](RenderQueue &self, int z_layer, const std::string &priority_group,
            ...
            self.add_task_text(z_layer, priority_group, text, font_id,
                               sdl_color, x, y);
          },
          nb::arg("z_layer"), nb::arg("priority_group"), nb::arg("x1"),
          nb::arg("y1"), nb::arg("x2"), nb::arg("y2"), nb::arg("color"),
      ...
      .def("get_sorted_priority_groups",
           &RenderQueue::get_sorted_priority_groups)
  ...
  nb::class_<ItemConfiguration>(m, "ItemConfiguration")
      ...
      .def("set_in_game_textures", &ItemConfiguration::setInGameTextures,
           nb::arg("textures"))
      .def("set_inventory_textures", &ItemConfiguration::setInventoryTextures,
           nb::arg("textures"))
      .def("set_default_value", &ItemConfiguration::setDefaultValue,
           nb::arg("key"), nb::arg("value"))
  ...
  m.def("register_item_configuration", &registerItemConfigurationOnManager,
        nb::arg("config"),
        "Register an ItemConfiguration and return a handle (uintptr_t).");
  m.def("get_item_configuration", &getItemConfigurationOnManager,
        nb::arg("item_id"), "Retrieve an ItemConfiguration by item_id.");
  ...
  nb::class_<GridData>(m, "GridData")
      .def(nb::init<>())
      .def(nb::init<int, int, int, float>(), nb::arg("terrainID"),
           nb::arg("entityID"), nb::arg("eventID"), nb::arg("lightingLevel"))
  ...
  nb::class_<ThreadError>(m, "ThreadError")
      .def(nb::init<int, const std::string &>(), nb::arg("thread_id"),
           nb::arg("error_message"))
      .def_ro("thread_id", &ThreadError::threadId,
              "Worker thread ID that encountered the error")
      .def_ro("error_message", &ThreadError::errorMessage,
              "Error message from the thread")
  ...
  nb::class_<World>(m, "World")
      ...
      .def("get_entity_by_id", &World::getEntityById,
           "Retrieve an EntityInterface by entity ID")
      ...
      .def("dispatch_move_entity_event_by_pos",
           ...
           nb::sig(
               "def dispatch_move_entity_event_by_pos(self, arg0: int, arg1: "
               "list[DirectionEnum], /) -> None: ..."))
      .def(
          "dispatch_move_entity_event_by_id",
      ...
      .def(
          "get_py_registry",
          [](World &w) -> PyRegistry * { return w.getPyRegistry(); },
          nb::rv_policy::reference_internal, nb::keep_alive<0, 1>(),
          "Borrowed PyRegistry view; keep World alive while this reference "
          "exists.")
      .def(
          "get_voxel_grid",
          [](World &w) -> VoxelGrid * { return w.getVoxelGrid(); },
          nb::rv_policy::reference_internal, nb::keep_alive<0, 1>(),
          "Borrowed VoxelGrid view; keep World alive while this reference "
          "exists.")
      // Updated method bindings for GameDBHandler interface with automatic type
      // conversion:
      .def("put_time_series",
           [](World &w, nb::object seriesName, nb::object timestamp,
              nb::object value) {
  ...
  nb::class_<WorldView>(m, "WorldView")
      ...
      .def("addEntity", &WorldView::addEntity, "Add an entity with a given ID",
           nb::arg("id"), nb::arg("entity"))
      .def("hasEntity", &WorldView::hasEntity,
           "Check if an entity exists by its ID", nb::arg("id"))
      ...
      .def("get_terrain_id", &WorldView::getTerrainId,
           "Retrieve an EntityInterface by coords",
           nb::rv_policy::reference_internal)
      .def("get_entity_id", &WorldView::getEntityId,
           "Retrieve an EntityInterface by coords",
           nb::rv_policy::reference_internal)
      .def("get_terrain", &WorldView::getTerrain,
           nb::rv_policy::reference_internal)
      .def("get_entity", &WorldView::getEntity,
           nb::rv_policy::reference_internal)
  ...
  nb::class_<WorldViewFlatB>(m, "WorldViewFlatB")
      ...
      .def("getWidth", &WorldViewFlatB::getWidth,
           nb::rv_policy::reference_internal)
      .def("getHeight", &WorldViewFlatB::getHeight,
           nb::rv_policy::reference_internal)
      .def("getDepth", &WorldViewFlatB::getDepth,
           nb::rv_policy::reference_internal)
      .def("getVoxelGrid", &WorldViewFlatB::getVoxelGrid,
           nb::rv_policy::reference_internal)
      .def("getEntityById", &WorldViewFlatB::getEntityById,
           nb::rv_policy::reference_internal)
      .def("get_terrain", &WorldViewFlatB::getTerrain,
           nb::rv_policy::reference_internal)
      .def("get_entity", &WorldViewFlatB::getEntity,
           nb::rv_policy::reference_internal);
  ...
  nb::class_<PerceptionResponse>(m, "PerceptionResponse")
      ...
      .def_rw("entity", &PerceptionResponse::entity,
              nb::rv_policy::reference_internal)
      .def_rw("world_view", &PerceptionResponse::world_view,
              nb::rv_policy::reference_internal);

  nb::class_<QueryResponse>(m, "QueryResponse")
      .def("serialize", &QueryResponse::py_serialize);
  ...
  nb::class_<MapOfMapsOfDoubleResponse, QueryResponse>(
      m, "MapOfMapsOfDoubleResponse")
      .def(nb::init<>())
      .def(nb::init<
  ...
  nb::class_<PerceptionResponseFlatB>(m, "PerceptionResponseFlatB")
      ...
      .def("getWorldView", &PerceptionResponseFlatB::getWorldView,
           nb::rv_policy::reference_internal)
      .def("getEntity", &PerceptionResponseFlatB::getEntity,
           nb::rv_policy::reference_internal)
      .def("get_item_from_inventory_by_id",
           &PerceptionResponseFlatB::getItemFromInventoryById,
           nb::rv_policy::reference_internal)
      .def("get_query_response_by_id",
           &PerceptionResponseFlatB::getQueryResponseById,
           nb::rv_policy::reference_internal)
      .def("get_ticks", &PerceptionResponseFlatB::getTicks,
           nb::rv_policy::reference_internal);
  ...
  nb::class_<PhysicsSettings>(m, "PhysicsSettings")
      ...
      .def("set_allow_multi_direction",
           &PhysicsSettings::setAllowMultiDirection)
      .def("set_metabolism_cost_to_apply_force",
           &PhysicsSettings::setMetabolismCostToApplyForce)
      .def("set_evaporation_coefficient",
           &PhysicsSettings::setEvaporationCoefficient)
      .def("set_heat_to_water_evaporation",
           &PhysicsSettings::setHeatToWaterEvaporation)
      ...
      .def("get_allow_multi_direction",
           &PhysicsSettings::getAllowMultiDirection)
      .def("set_simulate_vapor_condensation",
           &PhysicsSettings::setSimulateVaporCondensation)
      .def("set_simulate_vapor_movement",
           &PhysicsSettings::setSimulateVaporMovement)
      .def("set_simulate_water_movement",
           &PhysicsSettings::setSimulateWaterMovement)
      .def("set_simulate_water_evaporation",
           &PhysicsSettings::setSimulateWaterEvaporation)
      .def("get_simulate_vapor_condensation",
           &PhysicsSettings::getSimulateVaporCondensation)
      .def("get_simulate_vapor_movement",
           &PhysicsSettings::getSimulateVaporMovement)
  ...
  nb::class_<StructuralIntegrityComponent>(m, "StructuralIntegrityComponent")
      .def(nb::init<>())
      .def_rw("can_stack_entities",
              &StructuralIntegrityComponent::canStackEntities)
      .def_rw("max_load_capacity",
              &StructuralIntegrityComponent::maxLoadCapacity)
  ...
  nb::class_<Inventory>(m, "Inventory")
      ...
      .def("remove_item_by_slot", &Inventory::removeItemBySlot,
           nb::arg("slotID"))
  ...
  nb::class_<DropRates>(m, "DropRates")
      ...
      .def("add_item", &DropRates::addItem, nb::arg("itemID"),
           nb::arg("dropRate"), nb::arg("minDrop"), nb::arg("maxDrop"));
  ...
  nb::class_<TileEffectComponent>(m, "TileEffectComponent")
      ...
      .def_rw("effect_remaining_time",
              &TileEffectComponent::effectRemainingTime);
  ...
  nb::class_<EntityInterface>(m, "EntityInterface")
      ...
      .def("get_position",
           [](EntityInterface &self) -> const Position & {
             return self.getComponent<Position>();
           })
      .def("set_position",
           [](EntityInterface &self, const Position &value) {
             self.setComponent<Position>(value);
           })

      .def("get_velocity",
           [](EntityInterface &self) -> const Velocity & {
             return self.getComponent<Velocity>();
           })
      .def("set_velocity",
           [](EntityInterface &self, const Velocity &value) {
             self.setComponent<Velocity>(value);
           })
      ...
      .def(
          ...
          nb::rv_policy::reference_internal)
      .def("set_inventory",
           [](EntityInterface &self, const Inventory &value) {
             self.setComponent<Inventory>(value);
           })
      ...
      .def(
          "get_item_enum",
          [](EntityInterface &self) -> const ItemEnum & {
            return self.getComponent<ItemEnum>();
          },
          nb::rv_policy::reference_internal)
      .def("set_item_enum",
           [](EntityInterface &self, const ItemEnum &value) {
             self.setComponent<ItemEnum>(value);
           })
      .def(
          "get_item_type_comp",
      ...
      .def(
          "get_food_item",
          [](EntityInterface &self) -> const FoodItem & {
            return self.getComponent<FoodItem>();
          },
          nb::rv_policy::reference_internal)
      .def("set_food_item",
           [](EntityInterface &self, const FoodItem &value) {
             self.setComponent<FoodItem>(value);
           })
  ...
  nb::bind_map<std::unordered_map<int, EntityInterface>>(m,
                                                         "MapEntityInterface");
  ...
  nb::class_<VoxelGridCoordinates>(m, "VoxelGridCoordinates")
      ...
      .def(nb::self == nb::self);
  nb::bind_vector<std::vector<VoxelGridCoordinates>>(m,
                                                     "VecVoxelGridCoordinates");
  ...
  nb::class_<VoxelGrid>(m, "VoxelGrid")
      ...
      .def("get_voxel", &VoxelGrid::getVoxel, nb::arg("x"), nb::arg("y"),
           nb::arg("z"), "Get voxel data at (x, y, z)")
      .def("set_terrain", &VoxelGrid::setTerrain, nb::arg("x"), nb::arg("y"),
      ...
      .def("create_entt_for_terrain", &VoxelGrid::createEnttForTerrain,
           ...
           "ID")
      .def("set_entity", &VoxelGrid::setEntity, nb::arg("x"), nb::arg("y"),
           nb::arg("z"), nb::arg("entityID"), "Set entityID at (x, y, z)")
      .def("get_entity", &VoxelGrid::getEntity, nb::arg("x"), nb::arg("y"),
           nb::arg("z"), "Get entityID at (x, y, z)")
      .def("set_event", &VoxelGrid::setEvent, nb::arg("x"), nb::arg("y"),
           nb::arg("z"), nb::arg("eventID"), "Set eventID at (x, y, z)")
      .def("get_event", &VoxelGrid::getEvent, nb::arg("x"), nb::arg("y"),
           nb::arg("z"), "Get eventID at (x, y, z)")
      .def("set_lighting_level", &VoxelGrid::setLightingLevel, nb::arg("x"),
           nb::arg("y"), nb::arg("z"), nb::arg("lightingLevel"),
           "Set lightingLevel at (x, y, z)")
      .def("get_lighting_level", &VoxelGrid::getLightingLevel, nb::arg("x"),
           nb::arg("y"), nb::arg("z"), "Get lightingLevel at (x, y, z)")

      // Bind the new utility search methods
      .def("get_all_terrain_in_region", &VoxelGrid::getAllTerrainInRegion,
           nb::arg("x_min"), nb::arg("y_min"), nb::arg("z_min"),
           nb::arg("x_max"), nb::arg("y_max"), nb::arg("z_max"),
           "Retrieve all terrain voxel coordinates within a specified region.")
      .def("get_all_entity_in_region", &VoxelGrid::getAllEntityInRegion,
           nb::arg("x_min"), nb::arg("y_min"), nb::arg("z_min"),
           nb::arg("x_max"), nb::arg("y_max"), nb::arg("z_max"),
           "Retrieve all entity voxel coordinates within a specified region.")
      .def("get_all_event_in_region", &VoxelGrid::getAllEventInRegion,
           nb::arg("x_min"), nb::arg("y_min"), nb::arg("z_min"),
           nb::arg("x_max"), nb::arg("y_max"), nb::arg("z_max"),
           "Retrieve all event voxel coordinates within a specified region.")
      .def("get_all_lighting_in_region", &VoxelGrid::getAllLightingInRegion,
           nb::arg("x_min"), nb::arg("y_min"), nb::arg("z_min"),
           nb::arg("x_max"), nb::arg("y_max"), nb::arg("z_max"),
           "Retrieve all lighting voxel coordinates within a specified region.")

      .def("set_terrain_entity_type_component",
           &VoxelGrid::setTerrainEntityTypeComponent, nb::arg("x"),
           nb::arg("y"), nb::arg("z"), nb::arg("component"),
           "Set the EntityTypeComponent for the terrain entity at (x, y, z) "
           "based on its terrain type")
  ...
  nb::class_<GameClock>(m, "GameClock")
      ...
      .def("get_season", &GameClock::getSeason,
  ...
  nb::class_<PyRegistry>(m, "PyRegistry")
      // Remove the constructor binding since PyRegistry cannot be constructed
      // from Python
