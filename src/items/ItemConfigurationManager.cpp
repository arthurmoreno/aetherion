...
uintptr_t registerItemConfigurationOnManager(
    const std::shared_ptr<ItemConfiguration> &config) {
  auto &manager = ItemConfigurationManager::getInstance();

  if (manager.configurations.find(config->getItemId()) !=
      manager.configurations.end()) {
    throw std::runtime_error("ItemConfiguration with ID '" +
                             config->getItemId() + "' already exists.");
  }
...
std::shared_ptr<ItemConfiguration>
getItemConfigurationOnManager(const std::string &itemId) {
...
std::shared_ptr<ItemConfiguration>
ItemConfigurationManager::getItemConfiguration(
...
void ItemConfigurationManager::deregisterItemConfiguration(
    const std::string &itemId) {
  std::lock_guard<std::mutex> lock(managerMutex);
  configurations.erase(itemId);
}

ItemConfigurationManager::~ItemConfigurationManager() {
