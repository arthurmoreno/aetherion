...
public:
  ...
  std::unordered_map<std::string, std::shared_ptr<ItemConfiguration>>
      configurations;
  ...
  operator=(const ItemConfigurationManager &) = delete;

  // Registration methods
  void
  registerItemConfiguration(const std::shared_ptr<ItemConfiguration> &config);
  std::shared_ptr<ItemConfiguration>
  getItemConfiguration(const std::string &itemId) const;
