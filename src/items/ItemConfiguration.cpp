...
void ItemConfiguration::setInventoryTextures(
    const std::vector<std::string> &textures) {
  inventoryTextures = textures;
}

void ItemConfiguration::setDefaultValue(const std::string &key,
...
ItemConfiguration::getInventoryTextures() const {
  return inventoryTextures;
}

const ItemConfiguration::DefaultValue *
ItemConfiguration::getDefaultValue(const std::string &key) const {
...
entt::entity ItemConfiguration::createFoodItem(entt::registry &registry) {
  ...
  if (itemMainType == static_cast<int>(ItemEnum::FOOD)) {
    ...
    float energyHealthRatio =
        this->getDefaultValueAsFloat("energy_health_ratio");
    float conversionEfficiency =
        this->getDefaultValueAsFloat("conversion_efficiency");
    ...
    registry.emplace<FoodItem>(
        newFoodItem, FoodItem{.energyDensity = energyDensity,
                              .mass = defaultMass,
