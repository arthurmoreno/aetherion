...
struct WaveComponent {
  ...
  WaveComponent()
      : sourceId(-1), type(WaveType::SOUND), amplitude(0.0f), frequency(440.0f),
        speed(1.0f), attenuationPerUnit(0.1f), lifetimeTicks(0), dirX(0.0f),
        dirY(0.0f), dirZ(0.0f), distanceTraveled(0.0f) {}
};
...
struct WaveEmitterComponent {
  ...
  WaveEmitterComponent()
      : type(WaveType::SOUND), initialAmplitude(1.0f), frequency(440.0f),
        speed(1.0f), attenuationPerUnit(0.1f), maxLifetimeTicks(100),
        isActive(false) {}
};
...
struct SoundHeardEvent {
  ...
  SoundHeardEvent(entt::entity listener, int32_t source, WaveType waveType,
                  float frequency, float amplitude)
      : listener(listener), source(source), waveType(waveType),
...
struct WaveImpactEvent {
  ...
  WaveImpactEvent(entt::entity target, int32_t source, float impulseX,
                  float impulseY, float impulseZ, float amplitude)
      : target(target), source(source), impulseX(impulseX), impulseY(impulseY),
        impulseZ(impulseZ), amplitude(amplitude) {}
};
...
public:
  ...
  void processWaves(entt::registry &registry, VoxelGrid &grid,
                    entt::dispatcher &dispatcher, uint32_t dtTicks);

  // Emit a new wave from a position
  void emitWave(VoxelGrid &grid, int x, int y, int z,
...
private:
  ...
  void propagateWaves(entt::registry &registry, VoxelGrid &grid,
                      entt::dispatcher &dispatcher, uint32_t dtTicks);
  void propagateSpherical(
  ...
  void handleWaveInteractions(std::vector<WaveComponent> &waves, int x, int y,
                              int z);
  void processWaveInteractions(entt::registry &registry,
                               entt::dispatcher &dispatcher, int x, int y,
                               int z, const std::vector<WaveComponent> &waves);

  // Physics integration
  void applyPhysicsImpulse(entt::registry &registry,
  ...
  getSphericalNeighbors(int x, int y, int z, float radius) const;
};
