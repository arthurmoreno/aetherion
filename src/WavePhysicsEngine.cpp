...
uint64_t WavePhysicsEngine::coordToKey(int x, int y, int z) const {
  ...
  // Pack into 64-bit key (assuming coordinates fit in 16 bits each after
  // offset)
...
bool WavePhysicsEngine::isValidCoordinate(VoxelGrid &grid, int x, int y,
                                          int z) const {
  return x >= 0 && x < grid.width && y >= 0 && y < grid.height && z >= 0 &&
         z < grid.depth;
}

float WavePhysicsEngine::calculateAttenuation(const WaveComponent &wave,
                                              float distance) const {
  // Apply distance-based attenuation
  float attenuatedAmplitude =
      wave.amplitude - (wave.attenuationPerUnit * distance);
...
WavePhysicsEngine::getSphericalNeighbors(int x, int y, int z,
                                         float radius) const {
...
void WavePhysicsEngine::emitWave(VoxelGrid &grid, int x, int y, int z,
...
void WavePhysicsEngine::processEmitters(entt::registry &registry,
                                        VoxelGrid &grid) {
...
void WavePhysicsEngine::handleWaveInteractions(
    std::vector<WaveComponent> &waves, int x, int y, int z) {
  if (waves.empty())
    return;
  ...
  if (impactWaves.size() > 1) {
    ...
    waves.erase(std::remove_if(waves.begin(), waves.end(),
                               [](const WaveComponent &w) {
                                 return w.type == WaveType::IMPACT;
                               }),
                waves.end());
  }
}

void WavePhysicsEngine::processWaveInteractions(
    entt::registry &registry, entt::dispatcher &dispatcher, int x, int y, int z,
        ...
        if (pos.x == x && pos.y == y && pos.z == z) {
          for (const auto &wave : waves) {
            if (wave.amplitude >= receiver.hearingThreshold) {
              dispatcher.enqueue<SoundHeardEvent>(entity, wave.sourceId,
                                                  wave.type, wave.frequency,
                                                  wave.amplitude);
            }
          }
        }
  ...
  registry.view<Velocity, Position>().each(
      [&](auto entity, auto &velocity, auto &pos) {
        if (pos.x == x && pos.y == y && pos.z == z) {
          for (const auto &wave : waves) {
            if (wave.type == WaveType::IMPACT && wave.sourceId != -1) {
              ...
              if (registry.valid(entt::entity(wave.sourceId))) {
                auto *sourcePos =
                    registry.try_get<Position>(entt::entity(wave.sourceId));
                if (sourcePos) {
                  applyPhysicsImpulse(registry, dispatcher, wave, x, y, z,
...
void WavePhysicsEngine::applyPhysicsImpulse(entt::registry &registry,
                                            entt::dispatcher &dispatcher,
                                            const WaveComponent &wave, int x,
                                            int y, int z, int sourceX,
  ...
  float impulseMagnitude =
      wave.amplitude * 0.1f; // Scale factor for physics integration
  ...
  registry.view<Velocity, Position>().each(
      [&](auto entity, auto &velocity, auto &pos) {
        if (pos.x == x && pos.y == y && pos.z == z) {
          ...
          dispatcher.enqueue<WaveImpactEvent>(
              entity, wave.sourceId, dx * impulseMagnitude,
              dy * impulseMagnitude, dz * impulseMagnitude, wave.amplitude);
        }
      });
}
...
void WavePhysicsEngine::propagateSpherical(
  ...
  auto neighbors =
      getSphericalNeighbors(originX, originY, originZ, propagationRadius);
...
void WavePhysicsEngine::propagateWaves(entt::registry &registry,
                                       VoxelGrid &grid,
                                       entt::dispatcher &dispatcher,
                                       uint32_t dtTicks) {
