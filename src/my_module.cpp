...
static PyObject *add_scalar(PyObject *self, PyObject *args) {
  ...
  std::cout << "First element before CUDA operation: " << host_array[0]
            << std::endl;
  ...
  cudaMemcpy(device_array, host_array, n * sizeof(float),
  ...
  cudaMemcpy(host_array, device_array, n * sizeof(float),
             cudaMemcpyDeviceToHost);
  check_cuda_error("cudaMemcpy to host");

  std::cout << "First element after CUDA operation: " << host_array[0]
            << std::endl;
...
// Module method table
static PyMethodDef MyMethods[] = {{"add_scalar", (PyCFunction)add_scalar,
                                   METH_VARARGS,
