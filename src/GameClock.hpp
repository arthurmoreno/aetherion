...
private:
  ...
  static const uint64_t MINUTES_PER_HOUR =
      10; // Remove this one, just used for faster debugging
  static const uint64_t HOURS_PER_DAY = 24;
  static const uint64_t DAYS_PER_MONTH =
      28; // Also represents the days in a season
...
public:
  ...
  uint64_t getMinute() const {
    return (ticks / SECONDS_PER_MINUTE) % MINUTES_PER_HOUR;
  }
