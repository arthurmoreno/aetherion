...
nb::object fbGetQueryResponseById(
  ...
  if (!queryResponses
           .size()) { // Since it's a reference, check size instead of null
    return nb::none();
  }
  ...
  for (auto queryResponse : queryResponses) {
    if (queryResponse->query_id() == query_id) {
      ...
      // Deserialize into EntityInterface
      auto deserializedQueryResponse = QueryResponse::deserialize(
          queryResponseBuffer.data(), queryResponseBuffer.size());
