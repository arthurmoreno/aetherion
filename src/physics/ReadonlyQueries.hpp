...
DirectionEnum getDirectionFromVelocities(float velocityX, float velocityY,
                                         float velocityZ);

// Helper: Get EntityTypeComponent for terrain or ECS entity
inline EntityTypeComponent
getEntityType(entt::registry &registry, VoxelGrid &voxelGrid,
              entt::entity entity, const Position &position, bool isTerrain) {
  if (isTerrain) {
    return voxelGrid.terrainGridRepository->getTerrainEntityType(
...
inline MatterState getMatterState(entt::registry &registry,
                                  VoxelGrid &voxelGrid, entt::entity entity,
                                  const Position &position, bool isTerrain) {
  if (!isTerrain) {
...
inline EntityTypeComponent getEntityTypeComponent(entt::registry &registry,
                                                  VoxelGrid &voxelGrid,
                                                  entt::entity entity, int x,
                                                  int y, int z,
                                                  bool isTerrain) {
  if (isTerrain) {
...
bool checkIfCanFall(entt::registry &registry, VoxelGrid &voxelGrid, int i,
                    int j, int k);
bool checkIfTerrainCanFall(entt::registry &registry, VoxelGrid &voxelGrid,
                           int i, int j, int k, MatterState matterState);

bool getTypeAndCheckSoftEmpty(entt::registry &registry, VoxelGrid &voxelGrid,
...
inline bool checkBelowStability(entt::registry &registry, VoxelGrid &voxelGrid,
  ...
  int bellowEntityId =
      voxelGrid.getEntity(position.x, position.y, position.z - 1);
...
std::tuple<int, int, int, float> calculateMovementDestination(
    entt::registry &registry, VoxelGrid &voxelGrid, const Position &position,
    Velocity &velocity, const PhysicsStats &physicsStats, float newVelocityX,
    float newVelocityY, float newVelocityZ);

bool hasCollision(entt::registry &registry, VoxelGrid &voxelGrid,
                  entt::entity entity, int movingFromX, int movingFromY,
                  int movingFromZ, int movingToX, int movingToY, int movingToZ,
                  bool isTerrain);

// Helper: Print exhaustive terrain diagnostics for debugging
inline void printTerrainDiagnostics(entt::registry &registry,
                                    VoxelGrid &voxelGrid,
                                    entt::entity invalidTerrain,
                                    const Position &position,
                                    const EntityTypeComponent &terrainType,
                                    int vaporMatter) {
  ...
  StructuralIntegrityComponent structuralIntegrity =
      voxelGrid.terrainGridRepository->getTerrainStructuralIntegrity(
          position.x, position.y, position.z);
  std::cout << "\n--- StructuralIntegrityComponent ---\n";
  std::cout << "  matterState: "
            << static_cast<int>(structuralIntegrity.matterState) << "\n";
  ...
  std::optional<int> terrainIdInStorage =
      voxelGrid.terrainGridRepository->getTerrainIdIfExists(
          position.x, position.y, position.z);
  std::cout << "\n--- Storage Info ---\n";
  std::cout << "  Terrain exists in storage: "
            << (terrainIdInStorage.has_value() ? "YES" : "NO") << "\n";
  ...
  if (vaporMatter <= 0) {
    std::cout << "  REASON: vaporMatter is " << vaporMatter
              << " (must be > 0)\n";
  }
  std::cout << "==============================================================="
               "===\n\n";
...
calculateVelocityAfterGravityStep(entt::registry &registry,
