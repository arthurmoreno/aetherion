  ...
  if (PhysicsManager::Instance()->getAllowMultiDirection()) {
    ...
    if (absVx >= absVy && absVx >= absVz && // X has the greatest velocity
      ...
      newVz = 0.0f;
               ...
               absVy >= absVz && // Y has the greatest velocity
               absAy >= absAx &&
               absAy >= absAz // Y has the greatest acceleration
    ) {
      ...
      newVz = 0.0f;
               ...
               absVz >= absVy && // Z has the greatest velocity
               absAz >= absAx &&
               absAz >= absAy // Z has the greatest acceleration
    ) {
 ...
 */
inline int calculateTimeToMove(float velocityX, float velocityY,
                               float velocityZ) {
...
inline float calculateVelocityFromTime(int timeToMove) {
  // Handle the case where timeToMove is zero or extremely small to avoid
  // division by zero
  if (timeToMove > 0) {
...
std::pair<float, bool> calculateVelocityAfterFrictionStep(float velocity,
                                                          int dt);

// Helper: Apply friction to horizontal velocities
inline std::tuple<float, float, bool, bool>
applyKineticFrictionDamping(float velocityX, float velocityY,
                            MatterState matterState, bool bellowIsStable,
                            float newVelocityZ) {
  if (matterState == MatterState::SOLID && bellowIsStable &&
      newVelocityZ <= 0) {
