 ...
 * `entt` components (for example: `Position`, `Velocity`,
 * `EntityTypeComponent`, `StructuralIntegrityComponent`, and `MatterContainer`)
 * for a single entity.
 *
 ...
 */
void updateEntityVelocity(Velocity &velocity, float newVx, float newVy,
                          float newVz);

/**
 * @brief Ensures a terrain entity has a Position component, adding one if it's
 * missing.
 * @details This is a safety check for terrain entities (e.g., vapor) that might
 * be processed by physics before being fully initialized. It fetches the
 * position from the TerrainGridRepository.
 ...
 * @throws std::runtime_error if the entity is missing a position and it cannot
 * be found in the repository.
 */
void ensurePositionComponentForTerrain(entt::registry &registry,
                                       VoxelGrid &voxelGrid,
                                       entt::entity entity, bool isTerrain);

/**
 * @brief Converts an entity into a "soft empty" terrain block.
 * @details Used when an entity cannot be immediately destroyed because there
 * are ongoing tile effects; converts the entity's components so it behaves as
 * an empty tile while effects finish processing.
 * @param registry The entt::registry.
...
/**
 * @brief Sets or overwrites components of an entity in the ECS to represent an
 * empty water tile.
 * @param registry The entt::registry.
 * @param terrain The entity to modify.
 * @param matterState The matter state (e.g., LIQUID) to assign to the entity.
 */
void setEmptyWaterComponentsEnTT(entt::registry &registry,
                                 entt::entity &terrain,
