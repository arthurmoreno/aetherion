...
inline const std::string PHYSICS_WATER_CREATION = "physics_water_creation";
...
resolveVerticalMotion(entt::registry &registry, VoxelGrid &voxelGrid,
                      const Position &position, float velocityZ,
                      MatterState matterState, entt::entity entityBeingDebugged,
                      entt::entity entity) {
  if (matterState == MatterState::SOLID || matterState == MatterState::LIQUID) {
    // Silenced: gravity-application spam was crowding the log during
    // water-state debugging. Re-enable if movement-velocity instrumentation
    // is needed again.
    // if (entity == entityBeingDebugged) {
    //   std::cout << "handleMovement -> applying Gravity" << std::endl;
    // }
    auto resultZ = calculateVelocityAfterGravityStep(
        registry, voxelGrid, position.x, position.y, position.z, velocityZ, 1);
    float newVelocityZ = resultZ.first;
    resultZ = calculateVelocityAfterGravityStep(
        registry, voxelGrid, position.x, position.y, position.z, velocityZ, 2);
...
void PhysicsEngine::onVaporCreationEvent(const VaporCreationEvent &event) {
  ...
  if (terrainAboveId != static_cast<int>(TerrainIdTypeEnum::NONE)) {
    ...
    if (!(typeAbove.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
      ...
      oss << "[onVaporCreationEvent] Cannot add vapor above at (" << tx << ", "
          << ty << ", " << tz
          << ") - target invalid. type=" << typeAbove.mainType
          << ", subtype=" << typeAbove.subType0
          << ", WaterMatter=" << matterAbove.WaterMatter
  ...
  createVaporTerrainEntity(registry, *voxelGrid, event.position.x,
                           event.position.y, event.position.z, event.amount);
}
...
void PhysicsEngine::onVaporMergeUpEvent(const VaporMergeUpEvent &event) {
  ...
  // Only merge into a vapor/transitory tile: must be terrain WATER with no
  // liquid water
  if (targetType.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
    ...
    ossMessage << "[VaporMergeUpEvent] Merge target invalid at ("
               << event.target.x << ", " << event.target.y << ", "
               << event.target.z << ") - skipping merge."
               << " type=" << targetType.mainType
  ...
  if (registry.valid(event.sourceEntity)) {
    ...
    std::ostringstream ossMessage;
    ossMessage
        << "[VaporMergeUpEvent] Source entity invalid for vapor merge at ("
        << event.source.x << ", " << event.source.y << ", " << event.source.z
        << ")";
    spdlog::get("console")->debug(ossMessage.str());
  }
...
void PhysicsEngine::onAddVaporToTileAboveEvent(
  ...
  if (typeAbove.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
    ...
    voxelGrid->terrainGridRepository->setTerrainMatterContainer(
        x, y, z, matterContainerAbove);

    std::ostringstream ossMessage;
    ossMessage << "[AddVaporToTileAboveEvent] Added " << event.amount
               << " vapor at (" << x << ", " << y << ", " << z << ")";
    spdlog::get("console")->debug(ossMessage.str());
  } else {
    std::ostringstream ossMessage;
    ossMessage
        << "[AddVaporToTileAboveEvent] Cannot add vapor above; obstruction at ("
        << x << ", " << y << ", " << z << ")";
    spdlog::get("console")->debug(ossMessage.str());
  }
}
...
// Helper: Load entity data (Position, Velocity, PhysicsStats) from either ECS
...
loadEntityPhysicsData(entt::registry &registry, entt::dispatcher &dispatcher,
  ...
  if (isTerrain) {
    // std::cout << "[loadEntityPhysicsData] Loading terrain physics data for
    ...
    // First try to get Position from the ECS registry (source of truth for
    // current frame), then fall back to TerrainGridRepository's byEntity_ map
    // if not present in ECS. This avoids position mismatch where the repo map
    // has already been updated to a new position but the SIC data at the old
    // position is what we actually need.
    ...
    if (ecsPos) {
      ...
      try {
        ...
        // Entity not found in terrain repository - cleanup and re-throw with
        // more context
    ...
    if (!voxelGrid.checkIfTerrainExists(terrainPos.x, terrainPos.y,
                                        terrainPos.z)) {
      // Terrain not found in repository - perform cleanup and check grid
      // std::cout << "[loadEntityPhysicsData] Terrain not found in repository
      // at position ("
      //           << terrainPos.x << ", " << terrainPos.y << ", " <<
      //           terrainPos.z << ")\n";

      // Check if terrain exists in the underlying grid (OpenVDB storage)
      // It's possible the entity was deleted from entt but still exists in the
      // grid Terrain doesn't exist in grid either - cleanup entity and throw
      ...
      if (registry.valid(entity)) {
        ...
        throw std::runtime_error("Terrain does not exist at the given position "
                                 "in repository or grid");
      }

      // Terrain exists in grid but not in repository - return empty velocity
      // objects std::cout << "[loadEntityPhysicsData] Terrain found in grid but
      // not in repository. Returning empty velocity.\n";
      terrainVel = Velocity{0.f, 0.f, 0.f};
      terrainPS = voxelGrid.terrainGridRepository->getPhysicsStats(
          terrainPos.x, terrainPos.y, terrainPos.z);
      return std::tie(terrainPos, terrainVel, terrainPS);
    } else {
      // std::cout << "[loadEntityPhysicsData] Terrain found in repository at
      // position ("
      //           << terrainPos.x << ", " << terrainPos.y << ", " <<
      //           terrainPos.z << ")\n";
    }

    terrainVel = voxelGrid.terrainGridRepository->getVelocity(
        terrainPos.x, terrainPos.y, terrainPos.z);
    terrainPS = voxelGrid.terrainGridRepository->getPhysicsStats(
        terrainPos.x, terrainPos.y, terrainPos.z);
    return std::tie(terrainPos, terrainVel, terrainPS);
    ...
    if (!registry.valid(entity)) {
      throw std::runtime_error(
          "Entity no longer valid in loadEntityPhysicsData");
    }

    return std::tie(registry.get<Position>(entity),
...
void handleMovement(entt::registry &registry, entt::dispatcher &dispatcher,
  ...
  if (isTerrain) {
    logger->debug("[handleMovement] Handling terrain entity ID={}",
                  int(entity));
  }
  ...
  if (!registry.valid(entity)) {
    if (isTerrain) {
      logger->warn("[handleMovement][TERRAIN id={}] Entity INVALID in "
                   "registry, attempting recovery",
                   int(entity));
    }
    try {
      ...
      if (isTerrain) {
        logger->warn("[handleMovement][TERRAIN id={}] Entity recovery FAILED: "
                     "{} — SKIPPING",
                     int(entity), e.what());
      }
      return;
  ...
  // This ensures vapor entities are fully initialized before physics processes
  // them
  ...
  if (isTerrain) {
    ...
    if (sic.matterState == MatterState::LIQUID) {
      spdlog::get("console")->debug("[handleMovement][TERRAIN id={}] "
                                    "StructuralIntegrityComponent.matterState="
                                    "LIQUID - pos=({},{},{}), matterState={}",
                                    int(entity), pos.x, pos.y, pos.z,
                                    static_cast<int>(sic.matterState));
      // throw std::runtime_error("handleMovement: isTerrain and LIQUID - Just
      // checking...");
    }
  }
  ...
  if (isTerrain) {
    logger->debug(
        "[handleMovement][TERRAIN id={}] haveMovement(MovingComponent)={}",
        int(entity), haveMovement);
  }

  // ⚠️ CRITICAL FIX: Acquire terrain grid lock BEFORE reading any terrain data
  // to prevent TOCTOU race conditions where terrain moves between position
  // lookup and velocity/physics reads (see loadEntityPhysicsData lines 507-515)
  ...
  // Exception-safe lock release using try-catch with specific exception
  // handlers
  try {
    // Load entity physics data from ECS or terrain storage
    Position terrainPos{};
    ...
    // SAFETY CHECK 3: Load entity data (exceptions will propagate if issues
    ...
    MatterState matterState =
        getMatterState(registry, voxelGrid, entity, position, isTerrain);

    auto [newVelocityZ, willStopZ] =
        resolveVerticalMotion(registry, voxelGrid, position, velocity.vz,
                              matterState, entityBeingDebugged, entity);

    if (isTerrain && matterState == MatterState::LIQUID) {
      logger->debug("[handleMovement][TERRAIN id={}] resolveVerticalMotion: "
                    "newVelocityZ={:.2f} willStopZ={}",
    ...
    if (isTerrain && matterState == MatterState::LIQUID) {
      logger->debug("[handleMovement][TERRAIN id={}] bellowIsStable={}",
                    int(entity), bellowIsStable);
    }

    auto [newVelocityX, newVelocityY, willStopX, willStopY] =
        applyKineticFrictionDamping(velocity.vx, velocity.vy, matterState,
                                    bellowIsStable, newVelocityZ);

    if (isTerrain && matterState == MatterState::LIQUID) {
      logger->debug("[handleMovement][TERRAIN id={}] afterFriction: "
                    "newVel=({:.2f},{:.2f}) willStop=({},{})",
                    int(entity), newVelocityX, newVelocityY, willStopX,
                    willStopY);
    }
    ...
    if (matterState != MatterState::GAS) {
      ...
      if (isTerrain && matterState == MatterState::LIQUID) {
        logger->debug("[handleMovement][TERRAIN id={}] velocityUpdated: "
                      "vel=({:.2f},{:.2f},{:.2f})",
                      int(entity), velocity.vx, velocity.vy, velocity.vz);
      }
    } else {
      if (isTerrain) {
        logger->debug("[handleMovement][TERRAIN id={}] SKIPPED velocity update "
                      "(GAS state)",
                      int(entity));
      }
    }
    ...
    auto [movingToX, movingToY, movingToZ, completionTime] =
        calculateMovementDestination(registry, voxelGrid, position, velocity,
                                     physicsStats, velocity.vx, velocity.vy,
                                     velocity.vz);

    if (isTerrain) {
      int timeThreshold = calculateTimeToMove(physicsStats.minSpeed);
      logger->debug("[handleMovement][TERRAIN id={}] moveDest=({},{},{}) "
                    "completionTime={:.2f} timeThreshold={}",
                    int(entity), movingToX, movingToY, movingToZ,
                    completionTime, timeThreshold);
    }
    ...
    bool collision =
        hasCollision(registry, voxelGrid, entity, position.x, position.y,
                     position.z, movingToX, movingToY, movingToZ, isTerrain);

    if (isTerrain) {
      logger->debug("[handleMovement][TERRAIN id={}] collision={}", int(entity),
                    collision);
    }

    if (!collision &&
        completionTime < calculateTimeToMove(physicsStats.minSpeed)) {
      if (!haveMovement) {
        if (isTerrain) {
          logger->debug("[handleMovement][TERRAIN id={}] CREATING "
                        "MovingComponent: ({},{},{}) -> ({},{},{}) time={:.2f}",
        ...
        if (isTerrain) {
          logger->debug("[handleMovement][TERRAIN id={}] NO-OP: already has "
                        "MovingComponent",
                        int(entity));
        }
      }
      ...
      if (isTerrain) {
        bool timeExceeded =
            completionTime >= calculateTimeToMove(physicsStats.minSpeed);
        logger->debug(
            "[handleMovement][TERRAIN id={}] NOT MOVING: collision={} "
            "timeExceeded={} — trying lateral collision handler",
      ...
      if (isTerrain) {
        logger->debug(
            "[handleMovement][TERRAIN id={}] lateralCollision handled={}",
            int(entity), handled);
      }
      ...
      if (!handled) {
        ...
        if (isTerrain) {
          logger->debug(
              "[handleMovement][TERRAIN id={}] lateral NOT handled, vz zeroed",
              int(entity));
        }
      }

      // Clean up zero velocity
      cleanupZeroVelocity(registry, voxelGrid, entity, position, velocity,
                          isTerrain);

      if (isTerrain) {
        logger->debug(
            "[handleMovement][TERRAIN id={}] FINAL: vel=({:.2f},{:.2f},{:.2f}) "
            "— entity at rest or cleaned up",
            int(entity), velocity.vx, velocity.vy, velocity.vz);
      }
    }
    ...
    if (isTerrain) {
      logger->error("[handleMovement][TERRAIN id={}] TerrainLockException: {}",
                    int(entity), e.what());
    }
    std::cout << "[handleMovement] TerrainLockException: " << e.what()
              << std::endl;
    throw; // Re-throw after logging
    ...
    if (isTerrain) {
      logger->warn(
          "[handleMovement][TERRAIN id={}] InvalidTerrainMovementException: {}",
          int(entity), e.what());
    }
    std::cout << "[handleMovement] InvalidTerrainMovementException: "
    ...
    if (isTerrain) {
      logger->error("[handleMovement][TERRAIN id={}] PhysicsException: {}",
                    int(entity), e.what());
    }
    ...
    if (isTerrain) {
      logger->error(
          "[handleMovement][TERRAIN id={}] Unexpected exception occurred",
          int(entity));
    }
...
void handleMovingTo(entt::registry &registry, VoxelGrid &voxelGrid,
                    entt::entity entity, bool isTerrain) {
  // SAFETY CHECK: Validate entity is still valid
  if (!registry.valid(entity)) {
    std::ostringstream ossMessage;
    ossMessage << "[handleMovingTo] WARNING: Invalid entity "
               << static_cast<int>(entity) << " - skipping";
    spdlog::get("console")->debug(ossMessage.str());
    return;
  }
  ...
  if (!registry.all_of<MovingComponent, Position>(entity)) {
    std::ostringstream ossMessage;
    ossMessage << "[handleMovingTo] WARNING: Entity "
  ...
  if (isTerrain) {
    throw std::runtime_error("handleMovingTo: isTerrain - Just checking...");
    spdlog::get("console")->debug("[handleMovingTo] isTerrain Fetching matter "
                                  "state for terrain entity ID={}",
                                  int(entity));
    auto pos = registry.get<Position>(entity);
    StructuralIntegrityComponent sic =
        voxelGrid.terrainGridRepository->getTerrainStructuralIntegrity(
  ...
  auto &&[movingComponent, position] =
      registry.get<MovingComponent, Position>(entity);
  ...
  if (movingComponent.timeRemaining <= 0) {
    ...
    if (p_sic && !isTerrain) {
      ...
      // TODO: Remove me
      throw std::runtime_error("handleMovingTo: isTerrain - Just checking...");
      spdlog::get("console")->debug("[handleMovingTo] isTerrain Fetching "
                                    "matter state for terrain entity ID={}",
                                    int(entity));
      StructuralIntegrityComponent sic =
          voxelGrid.terrainGridRepository->getTerrainStructuralIntegrity(
    ...
    if (matterState == MatterState::SOLID) {
      ...
      resultZ = calculateVelocityAfterGravityStep(registry, voxelGrid,
    ...
    if (!hasVelocity) {
      ...
      //     std::ostringstream ossMessage;
      //     ossMessage
      //         << "[handleMovingTo] WARNING: Creating Velocity component for
      //         terrain entity
      //         "
    ...
    // CRITICAL: Remove MovingComponent to allow new movement events to be
    // processed std::cout << "[handleMovingTo] Removing MovingComponent from
    // entity "
  ...
  for (auto entity : movingComponentView) {
    ...
    if (!registry.all_of<Position>(entity)) {
      ...
      if (pos.x == -1 && pos.y == -1 && pos.z == -1) {
        {
          std::ostringstream ossMessage;
          ossMessage << "[processPhysics:MovingComponent] Could not find "
                        "position of entity "
                     << entityId
      ...
      {
        // std::ostringstream ossMessage;
        // ossMessage << "[processPhysics:MovingComponent] Found position of
        // entity " << entityId
        //           << " in TerrainGridRepository at (" << pos.x << ", " <<
        //           pos.y << ", "
        //           << pos.z
      ...
      EntityTypeComponent terrainType =
          voxelGrid.terrainGridRepository->getTerrainEntityType(pos.x, pos.y,
                                                                pos.z);
      int vaporMatter =
          voxelGrid.terrainGridRepository->getVaporMatter(pos.x, pos.y, pos.z);
      ...
      if (terrainType.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
        ...
        {
          std::ostringstream ossMessage;
          ossMessage << "[processPhysics:MovingComponent] Reviving cold vapor "
                        "terrain at ("
                     << pos.x << ", " << pos.y << ", " << pos.z
        ...
        {
          std::ostringstream ossMessage;
          ossMessage << "[processPhysics:MovingComponent] Revived vapor "
                        "terrain as entity "
                     << static_cast<int>(entity)
      ...
      } else {
        {
          std::ostringstream ossMessage;
          ossMessage
              << "[processPhysics:MovingComponent] Not vapor terrain (mainType="
    ...
    if (!registry.valid(entity)) {
      // std::cout << "[processPhysics] WARNING: Invalid entity in
      // movingComponentView - "
      //              "skipping; Entity ID="
      //           << static_cast<int>(entity) << std::endl;
  ...
  if (matterState == MatterState::SOLID || matterState == MatterState::LIQUID) {
    ...
    if (!isTerrain && registry.all_of<EntityTypeComponent>(entity)) {
      ...
      spdlog::get("console")->debug(
          ...
          "type.mainType: {}, type.subType0: {}, type.subType1: {}, "
          "isTerrain: {}, isAlreadyMoving: {}, physicsStats.mass: {:.2f}",
          static_cast<int>(entity), pos.x, pos.y, pos.z, type.mainType,
          type.subType0, type.subType1, isTerrain, isAlreadyMoving,
          physicsStats.mass);

      if (!isAlreadyMoving &&
          checkIfTerrainCanFall(registry, voxelGrid, pos.x, pos.y, pos.z,
                                matterState)) {
        ...
        spdlog::get("console")->debug(
            "Not enqueuing MoveSolidEntityEvent for terrain entity {} at "
            "position ({}, {}, {}), entity type.mainType: {} , "
            "type.subType0: {}, isTerrain: {}, isAlreadyMoving: {}",
            static_cast<int>(entity), pos.x, pos.y, pos.z, type.mainType,
            type.subType0, isTerrain, isAlreadyMoving);
      }
    } else {
      spdlog::get("console")->debug(
          "Entity {} at position ({}, {}, {}) is not terrain and does not "
          "have EntityTypeComponent, skipping physics processing. isTerrain: "
          "{}",
          static_cast<int>(entity), pos.x, pos.y, pos.z, isTerrain);
    }

  } else if (matterState == MatterState::GAS) {
    // Physics processing for gas entities (vapor)
    // Gas entities should be processed via EcosystemEngine for
    // buoyancy-driven movement This prevents duplicate event generation that
    // would cause cascading events Note: Vapor movement is driven by density
    // differences and is handled by the ecosystem engine during its vapor
    // movement phase, not by async gravity events
  }
}
...
void PhysicsEngine::processVelocityForEntity(entt::entity entity,
  ...
  if (!registry.all_of<Position>(entity)) {
      ...
      if (pos.x == -1 && pos.y == -1 && pos.z == -1) {
        {
          std::ostringstream ossMessage;
          ossMessage
              << "[processPhysics:Velocity] Could not find position of "
                 "entity "
              << static_cast<int>(entity)
    ...
    if (pos.x == -1 && pos.y == -1 && pos.z == -1) {
      {
        std::ostringstream ossMessage;
        ossMessage
            << "[processPhysics:Velocity] Could not find position of entity "
    ...
    {
      // std::ostringstream ossMessage;
      // ossMessage << "[processPhysics:Velocity] Found position of entity "
      // << entityId
      //            << " in TerrainGridRepository at (" << pos.x << ", " <<
      //            pos.y << ", "
      //            << pos.z
    ...
    if (terrainType.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
        vaporMatter > 0) {
      {
        std::ostringstream ossMessage;
        ossMessage
            << "[processPhysics:Velocity] Reviving cold vapor terrain at ("
            << pos.x << ", " << pos.y << ", " << pos.z
            << ") with vapor matter: " << vaporMatter;
        spdlog::get("console")->debug(ossMessage.str());
      }
...

      {
        std::ostringstream ossMessage;
        ossMessage
            << "[processPhysics:Velocity] Revived vapor terrain as entity "
  ...
  if (entityId == entityVoxelGridId) {
    try {
      ...
      std::ostringstream ossMessage;
      ossMessage << "[processPhysics] TerrainLockException for entity "
                 << entityId << ": " << e.what() << " - skipping";
      spdlog::get("console")->warn(ossMessage.str());
    } catch (const aetherion::PhysicsException &e) {
      std::ostringstream ossMessage;
  ...
  if (terrainVoxelGridId != static_cast<int>(TerrainIdTypeEnum::NONE) &&
    ...
    try {
      ...
      std::ostringstream ossMessage;
      ossMessage
          << "[processPhysics] InvalidEntityException for terrain entity "
          << entityId << ": " << e.what() << " - skipping";
      spdlog::get("console")->warn(ossMessage.str());
    } catch (const aetherion::TerrainLockException &e) {
      std::ostringstream ossMessage;
      ...
      std::ostringstream ossMessage;
      ossMessage << "[processPhysics] PhysicsException for terrain entity "
                 << entityId << ": " << e.what() << " - skipping";
      spdlog::get("console")->warn(ossMessage.str());
    }
  }
...
void PhysicsEngine::processVelocityForVDBVoxels(entt::registry &registry,
                                                VoxelGrid &voxelGrid) {
  ...
  for (auto &v : vdbVoxels) {
    processVelocityForVoxel(v.x, v.y, v.z, v.vx, v.vy, v.vz, registry,
                            voxelGrid);
  }
}

void PhysicsEngine::processVelocityForVoxel(int x, int y, int z, float vx,
                                            float vy, float vz,
                                            entt::registry &registry,
                                            VoxelGrid &voxelGrid) {
  try {
    ...
    voxelGrid.terrainGridRepository->setVelocity(x, y, z, vel);
    ...
    if (!collision && completionTime < calculateTimeToMove(ps.minSpeed)) {
      if (!voxelGrid.terrainGridRepository->hasMovingComponent(x, y, z)) {
        // The move-trigger sub-block lives in `_attemptVelocityDrivenMove`
        // (in `PhysicsMutators.hpp`). The mutator owns the locking, the
        // phase-mismatch guard, and the post-move wake-up nudge. It
        // returns `false` when it refused the move (in which case it has
        // already cleared the source velocity).
        _attemptVelocityDrivenMove(voxelGrid, pos, vel, toX, toY, toZ,
...
// Handle movement of gas (vapor) cells based on applied forces and
// environmental conditions. Coord-based: gas cells live only in the VDB
// grids of `TerrainGridRepository`, never in the ECS registry.
// Algorithm:
// 1. Validate voxelGrid exists
// 2. Acquire terrain grid lock for thread-safe access
// 3. Get terrain at event position (exit early if NONE; sanity-check the
//    cell's terrain id still matches the event's payload)
// 4. Read terrain data atomically: position, physics stats, velocity
// 5. Check if the cell already has a MovingComponent (repository as source
//    of truth)
// 6. Calculate acceleration from forces:
//    - X and Y from applied forces (F = ma)
//    - Z from buoyancy (density difference with environment)
// 7. Translate physics acceleration to grid velocities with max-speed
//    limits
// 8. Determine movement direction from new velocities
// 9. Decide if force can be applied:
...
void PhysicsEngine::onMoveGasEntityEvent(const MoveGasEntityEvent &event) {
  ...
  // The gas-movement path is purely coord-based after C4.x: vapor lives
  // only in the VDB grids of `TerrainGridRepository`, never in the ECS
  // registry. All reads and the velocity write below go through the
  // repository keyed by `event.position`. The `event.entity` field is
  // retained on the event struct for legacy reasons but is otherwise
  // unused inside this body.
  ...
  PhysicsStats physicsStats =
      voxelGrid->terrainGridRepository->getPhysicsStats(pos.x, pos.y, pos.z);
  Velocity velocity =
      voxelGrid->terrainGridRepository->getVelocity(pos.x, pos.y, pos.z);
  ...
  // Step 10: Decide if force can be applied (check direction constraints).
  // The repository-stored MovingComponent is the single source of truth for
  // the cell's current direction.
  if (haveMovement) {
    MovingComponent existingMc =
        voxelGrid->terrainGridRepository->getMovingComponent(pos.x, pos.y,
                                                             pos.z);
    canApplyForce = (direction == existingMc.direction);
  ...
  // Step 11: If force can be applied, persist the updated velocity to the
  // terrain grid. There is no ECS Velocity / MovingComponent mirror to
  // sync — the repository owns that state for terrain cells.
  if (canApplyForce) {
...
void PhysicsEngine::onMoveSolidEntityEvent(const MoveSolidEntityEvent &event) {
  ...
  if (registry.valid(event.entity) &&
      registry.all_of<Position, EntityTypeComponent, PhysicsStats>(
          event.entity)) {
    ...
    if (haveMovement) {
      ...
      (direction == movingComponent.direction) ? canApplyForce = true
                                               : canApplyForce = false;
    }
    ...
    if (canApplyForce) {
      if (registry.all_of<MetabolismComponent>(event.entity)) {
        ...
        float metabolismCost =
            physicsStats.mass *
            (std::abs(event.forceX) + std::abs(event.forceY) +
             std::abs(event.forceZ)) *
      ...
      velocity.vy = newVelocityY;
      velocity.vz = newVelocityZ;
      if (direction != DirectionEnum::UPWARD and
    ...
    // std::ostringstream ossMessage;
    // ossMessage << "Entity " << static_cast<int>(event.entity) << " lacks
    // required components."; spdlog::get("console")->debug(ossMessage.str());
  }
}
...
void PhysicsEngine::onMoveSolidLiquidTerrainEvent(
  ...
  if (registry.valid(event.entity) && registry.all_of<Position>(event.entity)) {
    ...
    if (!terrainId.has_value()) {
      spdlog::get("console")->warn(
          "onMoveSolidLiquidTerrainEvent -> No terrain found at position ({}, "
          "{}, {}) for entity {} - skipping event",
          pos.x, pos.y, pos.z, static_cast<int>(event.entity));
      return;
    } else if (terrainId.value() != static_cast<int>(event.entity)) {
      spdlog::get("console")->warn(
          "onMoveSolidLiquidTerrainEvent -> Terrain ID {} at position ({}, {}, "
          "{}) does not match event entity {} - skipping event",
          terrainId.value(), pos.x, pos.y, pos.z,
          static_cast<int>(event.entity));
      return;
    } else {
      spdlog::get("console")->debug(
          "onMoveSolidLiquidTerrainEvent -> Found terrain with ID {} at "
          "position ({}, {}, {}) for entity {}",
          terrainId.value(), pos.x, pos.y, pos.z,
          static_cast<int>(event.entity));
    }
    ...
    if (physicsStats.mass == 0.0f &&
      ...
      spdlog::get("console")->debug(
          "onMoveSolidLiquidTerrainEvent -> entity={} at pos=({}, {}, {}) has "
          "zero mass but is water terrain - assigning mass based on water "
          "matter (WaterMatter={})",
          static_cast<int>(event.entity), pos.x, pos.y, pos.z,
          matterContainer.WaterMatter);
      if (matterContainer.WaterMatter == 0) {
        throw std::runtime_error(
            "onMoveSolidLiquidTerrainEvent: Water terrain entity has zero mass "
            "and zero water matter, cannot assign mass");
      }
      physicsStats.mass = static_cast<float>(matterContainer.WaterMatter);
    } else if (physicsStats.mass == 0.0f) {
      spdlog::get("console")->warn("onMoveSolidLiquidTerrainEvent -> entity={} "
                                   "at pos=({}, {}, {}), type=({}, {}, {})",
                                   static_cast<int>(event.entity), pos.x, pos.y,
                                   pos.z, type.mainType, type.subType0,
                                   type.subType1);
      spdlog::get("console")->warn(
          "onMoveSolidLiquidTerrainEvent -> entity={} at pos=({}, {}, {}) has "
          "zero mass and is not water terrain - this may lead to unexpected "
          "behavior when applying forces",
          static_cast<int>(event.entity), pos.x, pos.y, pos.z);
      throw std::runtime_error(
          "onMoveSolidLiquidTerrainEvent: Entity has zero mass and is not "
          "water terrain, cannot apply forces");
    }

    spdlog::get("console")->debug("onMoveSolidLiquidTerrainEvent -> entity={} "
                                  "| pos=({}, {}, {}) dir={} | "
                                  "physicsStats: mass={}, maxSpeed={} | "
                                  "entityType: mainType={}, subType0={}",
                                  static_cast<int>(event.entity), pos.x, pos.y,
                                  pos.z, static_cast<int>(pos.direction),
                                  physicsStats.mass, physicsStats.maxSpeed,
                                  type.mainType, type.subType0);
    ...
    spdlog::get("console")->debug("onMoveSolidLiquidTerrainEvent -> entity={} "
                                  "| pre-existing velocity=({}, {}, {}) | "
                                  "hasVelocity={} | haveMovement={}",
                                  static_cast<int>(event.entity), velocity.vx,
                                  velocity.vy, velocity.vz, hasVelocity,
                                  haveMovement);
    ...
    spdlog::get("console")->debug(
        ...
        "acceleration=({}, {}, {}) (NOTE: accZ forced to 0, event.forceZ={} is "
        "IGNORED) | "
        "allowMultiDirection={}",
        static_cast<int>(event.entity), accelerationX, accelerationY,
    ...
    spdlog::get("console")->debug(
        ...
        static_cast<int>(event.entity), newVelocityX, newVelocityY,
        newVelocityZ, velocity.vx, velocity.vy, velocity.vz, accelerationX,
    ...
    if (haveMovement) {
      ...
      spdlog::get("console")->debug(
          "onMoveSolidLiquidTerrainEvent -> entity={} | "
          "direction check: newDirection={} vs movingComponent.direction={} -> "
          "canApplyForce={}",
          static_cast<int>(event.entity), static_cast<int>(direction),
          static_cast<int>(movingComponent.direction), canApplyForce);
    }
    // If velocities are zero, retain the current direction
    ...
    if (canApplyForce) {
      spdlog::get("console")->debug(
          ...
          newVelocityX, newVelocityY, newVelocityZ, static_cast<int>(direction),
    ...
    spdlog::get("console")->error(
        "onMoveSolidLiquidTerrainEvent -> entity={} is invalid or missing "
        "Position component | "
        "valid={} hasPosition={}",
        static_cast<int>(event.entity), registry.valid(event.entity),
...
void PhysicsEngine::onTakeItemEvent(const TakeItemEvent &event) {
  ...
  if (registry.valid(event.entity) &&
      ...
      int entityId = event.voxelGrid.getEntity(takingFromX, takingFromY,
  takingFromZ);
      ...
      if (entityId != -1) {
          ...
          if (registry.all_of<EntityTypeComponent, Inventory>(takenEntity)) {
              ...
              if (takingFromType.mainType == 1 &&
  takingFromInventory.itemIDs.size() > 0) { if
  (!takingFromInventory.itemIDs.empty()) { if (!inventory.isFull()) { int itemId
  = takingFromInventory.itemIDs.back();  // Retrieve the last item
                          takingFromInventory.itemIDs.pop_back();  // Remove the
  last item inventory.addItem(itemId);

                          if
  (registry.all_of<ConsoleLogsComponent>(event.entity)) { ConsoleLogsComponent&
  consoleLogs = registry.get<ConsoleLogsComponent>(event.entity);
  ...
  */
  ...
  auto all_view =
      registry
          .view<Position, EntityTypeComponent, Inventory, OnTakeItemBehavior>();

  if (registry.valid(event.entity) &&
      registry.all_of<Position, EntityTypeComponent, Inventory,
                      OnTakeItemBehavior>(event.entity)) {
    auto &&[pos, type, inventory, onTakeItemBehavior] =
        registry
            .get<Position, EntityTypeComponent, Inventory, OnTakeItemBehavior>(
                event.entity);
...
void PhysicsEngine::onUseItemEvent(const UseItemEvent &event) {
  auto all_view =
      registry
          .view<Position, EntityTypeComponent, Inventory, OnUseItemBehavior>();

  if (registry.valid(event.entity) &&
      registry.all_of<Position, EntityTypeComponent, Inventory,
                      OnUseItemBehavior>(event.entity)) {
    auto &&[pos, type, inventory, onUseItemBehavior] =
        registry
            .get<Position, EntityTypeComponent, Inventory, OnUseItemBehavior>(
                event.entity);

    nb::gil_scoped_acquire acquire;
    int entityId = static_cast<int>(event.entity);

    onUseItemBehavior.behavior(entityId, event.pyRegistryObj, event.voxelGrid,
                               event.itemSlot, event.hoveredEntityId,
                               event.selectedEntityId);
  }
}

void PhysicsEngine::onSetPhysicsEntityToDebug(
    const SetPhysicsEntityToDebug &event) {
...
void PhysicsEngine::onEvaporateWaterEntityEvent(
  ...
  // Lock terrain grid for atomic state change (includes PhysicsStats +
  // evaporation)
  ...
  if (canEvaporate) {
    ...
    if (physicsStats.heat > HEAT_TO_WATER_EVAPORATION) {
      ...
      voxelGrid->terrainGridRepository->setTerrainMatterContainer(
          pos.x, pos.y, pos.z, matterContainer);
      voxelGrid->terrainGridRepository->setPhysicsStats(pos.x, pos.y, pos.z,
                                                        physicsStats);

      // Create or add vapor on z+1
      addOrCreateVaporAbove(registry, *voxelGrid, pos.x, pos.y, pos.z,
                            waterEvaporated);
    } else {
...
void PhysicsEngine::onCondenseWaterEntityEvent(
  ...
  if (vaporMatter.WaterVapor < event.condensationAmount) {
    ...
    std::ostringstream ossMessage;
    ossMessage
        << "[onCondenseWaterEntityEvent] Not enough vapor to condense at (" << x
  ...
  if (vaporMatter.WaterMatter > 0 || vaporMatter.WaterVapor == 0) {
    std::ostringstream ossMessage;
    ossMessage << "[onCondenseWaterEntityEvent] Invalid vapor state for "
                  "condensation at ("
               << x << ", " << y << ", " << z
  ...
  spdlog::get("console")->debug(
      "[onCondenseWaterEntityEvent] Attempting to condense vapor at (" +
  ...
  if (event.terrainBelowId != static_cast<int>(TerrainIdTypeEnum::NONE)) {
    ...
    MatterContainer matterBelow =
        voxelGrid->terrainGridRepository->getTerrainMatterContainer(x, y,
                                                                    z - 1);
    ...
    if (typeBelow.mainType == static_cast<int>(EntityEnum::TERRAIN) &&
      ...
      std::ostringstream ossMessage;
      ossMessage << "[onCondenseWaterEntityEvent] Condensed "
                 << event.condensationAmount << " vapor at (" << x << ", " << y
                 << ", " << z << ")\n"
                 << "  ------------------------------------------------\n"
                 << "    matterBelow now has WaterMatter: "
                 << matterBelow.WaterMatter << "\n"
                 << "    matterBelow now has WaterVapor: "
                 << matterBelow.WaterVapor << "\n"
                 << "  ------------------------------------------------\n"
                 << "    vaporMatter now has WaterMatter: "
                 << vaporMatter.WaterMatter << "\n"
                 << "    vaporMatter now has WaterVapor: "
                 << vaporMatter.WaterVapor << "\n"
                 << "  ------------------------------------------------\n"
    ...
    std::ostringstream ossMessage;
    ossMessage << "[onCondenseWaterEntityEvent] No terrain below vapor at ("
               << x << ", " << y << ", " << z
...
// Water fall event handler — coord-based.
//
// `event.entity` is advisory: post-C1–C4 the fall path neither needs nor
// uses an EnTT handle to populate the destination cell.
// `createWaterTerrainFromFall` writes ON_GRID_STORAGE directly. The handler
// only inspects the entity field to detect a stale reference (an event
// queued before the referenced entity was destroyed) so it can repair the
...
void PhysicsEngine::onWaterFallEntityEvent(const WaterFallEntityEvent &event) {
  ...
  const bool entityFieldIsRealHandle =
      rawEntityId != static_cast<int>(TerrainIdTypeEnum::NONE) &&
  ...
  // and is not in scope for this change.
}
