...
public:
  ...
  WorldViewFlatB(const GameEngine::WorldView *fbWorldView,
                 const bool prePopulateEntities)
  ...
  WorldViewFlatB(nb::bytes serialized_data) {
    ...
    fbWorldView = GameEngine::GetWorldView(
        serialized_buffer.data()); // Deserialize FlatBuffer
  ...
  VoxelGridViewFlatB getVoxelGrid() const {
    return VoxelGridViewFlatB(fbWorldView->voxelGrid());
  }
...
struct WorldView {
  ...
  std::unordered_map<int, EntityInterface>
      entities; // Map of entity IDs to their interfaces
  std::unordered_map<int, EntityInterface>
  ...
  int getTerrainId(int x, int y, int z) const {
    return voxelGridView.getTerrainVoxel(x, y, z);
  }

  int getEntityId(int x, int y, int z) const {
    return voxelGridView.getEntityVoxel(x, y, z);
  }
  ...
  // TODO: There is a bug here, because voxelGridView is not set to -1, instead
  // it is set to 0
  ...
  // Get the terrain ID at a specific voxel
  nb::object getTerrain(int x, int y, int z) const {
    int entityId = voxelGridView.getTerrainVoxel(
        x, y, z); // Returns the terrain ID from the VoxelGrid
    if (entityId != -1) {
      return pyGetEntityById(
          entityId); // Map the entity ID to an EntityInterface object
    }
  ...
  // Get the entity object at a specific voxel (by entity ID)
  nb::object getEntity(int x, int y, int z) const {
    int entityId = voxelGridView.getEntityVoxel(
        x, y, z); // Get the entity ID from the VoxelGrid
    // return entityId;
    if (entityId != -1) {
      return pyGetEntityById(
          entityId); // Map the entity ID to an EntityInterface object
  ...
  static WorldView deserializeFlatBuffers(WorldViewFlatB worldViewFlatB) {
    ...
    if (fbVoxelGrid) {
      worldView.voxelGridView =
          VoxelGridView::deserializeFlatBuffers(fbVoxelGrid);
    }
    ...
    if (fbEntities) {
      for (auto fbEntity : *fbEntities) {
        ...
        EntityInterface deserializedEntity = EntityInterface::deserialize(
            entity_buffer.data(), entity_buffer.size());
  ...
  static WorldView pyDeserializeFlatBuffer(nb::bytes serialized_data) {
    ...
    auto fbWorldView =
        GameEngine::GetWorldView(data_ptr); // Deserialize FlatBuffer
  ...
  flatbuffers::Offset<GameEngine::WorldView>
  serializeFlatBuffer(flatbuffers::FlatBufferBuilder &builder) const {
    // Serialize voxelGrid
    auto voxelGridViewOffset = voxelGridView.serializeFlatBuffers(builder);

    std::vector<flatbuffers::Offset<GameEngine::EntityInterface>>
        entityOffsets = serializeEntities(builder, entities);
    auto entitiesOffset = builder.CreateVector(entityOffsets);

    auto worldViewOffset = GameEngine::CreateWorldView(
        builder, width, height, depth, voxelGridViewOffset, entitiesOffset);
