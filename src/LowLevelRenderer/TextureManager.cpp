...
void render_texture(uintptr_t renderer_ptr, uintptr_t texture_ptr, int x,
                    int y) {
  SDL_Renderer *renderer = reinterpret_cast<SDL_Renderer *>(renderer_ptr);
  ...
  if (SDL_RenderCopy(renderer, texture, nullptr, &dst_rect) != 0) {
    throw std::runtime_error(std::string("SDL_RenderCopy failed: ") +
                             SDL_GetError());
  }
}
...
void loadTextureOnManager(uintptr_t renderer_ptr, const std::string &imagePath,
  ...
  TextureManager::Instance()->load(imagePath, id, renderer, newWidth,
                                   newHeight);
}

void renderTextureFromManager(uintptr_t renderer_ptr, const std::string &id,
                              int x, int y) {
  SDL_Renderer *renderer = reinterpret_cast<SDL_Renderer *>(renderer_ptr);

  // Get texture width and height
  int w, h;
  if (SDL_QueryTexture(TextureManager::Instance()->m_textureMap[id], nullptr,
                       nullptr, &w, &h) != 0) {
    throw std::runtime_error(std::string("SDL_QueryTexture failed: ") +
                             SDL_GetError());
  }
  ...
  // Render the texture
  if (SDL_RenderCopy(renderer, TextureManager::Instance()->m_textureMap[id],
                     nullptr, &dst_rect) != 0) {
    throw std::runtime_error(std::string("SDL_RenderCopy failed: ") +
                             SDL_GetError());
  }
...
SDL_Texture *getTextureFromManager(const std::string &id) {
  ...
  if (it != TextureManager::Instance()->m_textureMap.end()) {
    ...
    std::cerr << "Texture with ID '" << id << "' not found in TextureManager."
              << std::endl;
    return nullptr;
  }
}
...
bool TextureManager::load(std::string fileName, std::string id,
                          SDL_Renderer *pRenderer, int newWidth,
                          int newHeight) {
  ...
  if (pTexture != nullptr) {
    if (newWidth > 0 && newHeight > 0) {
      ...
      SDL_Texture *pScaledTexture =
          SDL_CreateTexture(pRenderer, SDL_PIXELFORMAT_RGBA8888,
      ...
      if (SDL_QueryTexture(pTexture, nullptr, nullptr, &srcRect.w,
                           &srcRect.h) != 0) {
        SDL_DestroyTexture(pTexture);
        SDL_DestroyTexture(pScaledTexture);
...
void TextureManager::initOpenGL() {
  ...
  std::cout << "OpenGL TextureManager initialized successfully (OpenGL 1.1)"
            << std::endl;
}
...
bool TextureManager::loadGL(std::string fileName, std::string id,
                            uintptr_t gl_context_ptr, int newWidth,
                            int newHeight) {
  ...
  if (!rgbaSurface) {
    std::cerr << "Failed to convert surface to RGBA: " << SDL_GetError()
              << std::endl;
    return false;
  }
  ...
  std::cout << "OpenGL texture '" << id
            << "' loaded successfully (ID: " << texture << ")" << std::endl;
  return true;
}

void TextureManager::renderGL(std::string id, int x, int y, int width,
                              int height) {
  if (!s_openglInitialized) {
    initOpenGL();
  }
  ...
  glOrtho(0, viewport[2], viewport[3], 0, -1,
          1); // Top-left origin, like screen coordinates
...
GLuint load_texture_gl(uintptr_t gl_context_ptr,
  ...
  if (!rgbaSurface) {
    throw std::runtime_error(
        std::string("Failed to convert surface to RGBA: ") + SDL_GetError());
  }
...
void render_texture_gl(uintptr_t gl_context_ptr, GLuint texture_id, int x,
...
void loadTextureOnManagerGL(uintptr_t gl_context_ptr,
                            const std::string &imagePath, const std::string &id,
                            int newWidth, int newHeight) {
  TextureManager::Instance()->loadGL(imagePath, id, gl_context_ptr, newWidth,
                                     newHeight);
}

void renderTextureFromManagerGL(uintptr_t gl_context_ptr, const std::string &id,
