...
public:
  ...
  void destroy_node_only(entt::entity e) {
    ...
    entt::entity before =
        h.next_sibling; // reinsert children before where 'e' was
  ...
  void attach_child(entt::entity parent, entt::entity child,
    ...
    assert(!is_descendant_of_(parent, child) &&
           "Cannot create cycles in scene graph");
    ...
    // Root insertion
    if (parent == entt::null) {
      // Insert into roots_ before 'before' if before is also a root, else
      // append.
      if (before != entt::null) {
  ...
  void attach_node_python_instance(entt::entity node, nb::object instance) {
    if (!registry_->valid(node) || !registry_->all_of<Hierarchy>(node)) {
      throw std::runtime_error(
          "Entity is not a valid node in the scene graph.");
    }
  ...
  static constexpr bool is_null(entt::entity e) noexcept {
    return e == entt::null;
  }
  ...
  template <typename Fn> void for_each_child(entt::entity e, Fn &&fn) const {
    if (!contains(e))
      return;
    for (entt::entity c = first_child(e); c != entt::null;
         c = next_sibling(c)) {
      fn(c);
    }
  }
  ...
  void for_each_descendant_preorder(entt::entity root, Fn &&fn) const {
    ...
    while (!stack.empty()) {
      ...
      for (entt::entity c = first_child(cur); c != entt::null;
           c = next_sibling(c)) {
        temp.push_back(c);
      }
  ...
  // Returns a list of node IDs in the correct order for rendering (preorder
  // traversal)
...
private:
  ...
  bool is_descendant_of_(entt::entity candidate,
                         entt::entity ancestor) const noexcept {
    if (candidate == entt::null || ancestor == entt::null)
      return false;
