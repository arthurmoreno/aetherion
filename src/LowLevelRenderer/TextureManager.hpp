...
// #include <imgui/backends/imgui_impl_sdlrenderer2.h>  // Updated backend
// header #include <imgui/imgui.h>
...
void render_texture_gl(uintptr_t gl_context_ptr, GLuint texture_id, int x,
                       int y, int width = -1, int height = -1);
...
public:
  ...
  // OpenGL methods
  static bool loadGL(std::string fileName, std::string id,
                     uintptr_t gl_context_ptr, int newWidth = -1,
                     int newHeight = -1);
  static void renderGL(std::string id, int x, int y, int width = -1,
                       int height = -1);
  ...
  // currentRow, int currentFrame, SDL_Renderer *pRenderer, SDL_RendererFlip
  // flip = SDL_FLIP_NONE);
