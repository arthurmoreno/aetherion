 ...
 * a dependency injection container, providing programs with access to world
 * state, physics settings, inventory data, and entity interfaces without tight
 * coupling.
 */
struct GuiContext {
  // World state metrics
  ...
  nb::dict
      &sharedData; // Shared state dictionary for inter-module communication

  // Entity interfaces for inspection
  std::shared_ptr<EntityInterface>
      entityInterfacePtr; // Generic entity interface
  std::shared_ptr<EntityInterface>
      hoveredEntityInterfacePtr; // Currently hovered entity
  std::shared_ptr<EntityInterface>
      selectedEntityInterfacePtr; // Currently selected entity
};
