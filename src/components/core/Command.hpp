 ...
 *
 * This class uses nanobind::dict for maximum flexibility and seamless Python
 * interop. Parameters can be of any type that nanobind supports.
 *
 * Usage Example (C++):
 * @code
...
public:
   ...
   */
  nb::object operator[](const char *key) { return params_[key]; }

  /**
   * @brief Access parameter by key (const version)
   */
  nb::object operator[](const char *key) const { return params_[key]; }
   ...
   */
  template <typename T> void setParam(std::string_view key, T &&value) {
   ...
   * @throws std::runtime_error or nb::cast_error if parameter doesn't exist or
   * type mismatch
   */
  template <typename T> [[nodiscard]] T getParam(std::string_view key) const {
    if (!params_.contains(key.data())) {
      throw std::runtime_error("Parameter '" + std::string(key) +
                               "' not found in command '" + type_ + "'");
    }
...
public:
  ...
  [[nodiscard]] std::string getSrcWindow() const {
    return getParam<std::string>("src_window");
  }
...
public:
  ...
  explicit EditorCommand(Action action)
      : Command(std::string(actionToString(action))) {}

  [[nodiscard]] static constexpr std::string_view
  actionToString(Action action) noexcept {
    switch (action) {
    case Action::Play:
      return "editor_play";
    case Action::Stop:
    ...
    case Action::Step:
      return "editor_step";
    case Action::ExitToEditor:
      return "exit_to_editor";
    }
