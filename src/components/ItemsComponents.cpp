...
std::pair<int, int> splitStringToInts(const std::string &input) {
  ...
  if (colonPos == std::string::npos) {
    throw std::invalid_argument(
        "Input string must be in the format 'int:int'.");
  }
...
bool Inventory::isEmpty() const {
  return std::all_of(itemIDs.begin(), itemIDs.end(),
...
int Inventory::currentItemCount() const {
  return std::count_if(itemIDs.begin(), itemIDs.end(),
                       [](const int &id) { return id != -1; });
}
...
void Inventory::clearInventory() {
  std::fill(itemIDs.begin(), itemIDs.end(), -1);
}
...
void DropRates::addItem(std::string itemID, float dropRate, int minDrop,
                        int maxDrop) {
  // Validate dropRate
  ...
  if (minDrop < 0 || maxDrop < 0) {
    throw std::invalid_argument(
        "minDrop and maxDrop must be non-negative integers.");
  }
  if (minDrop > maxDrop) {
