...
int DigestionComponent::addItem(int itemID, int processingTime,
                                float energyLeft, float energyDensity,
...
void DigestionComponent::removeItem(int itemID) {
  auto it = std::find_if(digestingItems.begin(), digestingItems.end(),
                         [itemID](const DigestingFoodItem &item) {
                           return item.foodItemID == itemID;
                         });
