...
void EffectsSystem::processEffects(entt::registry &registry,
                                   VoxelGrid &voxelGrid,
  ...
  for (auto entity : tileEffectsListView) {
    ...
    if (tileEffectsList.tileEffectsIDs.empty()) {
      ...
      bool emptyMatter = (matterContainer.TerrainMatter == 0 &&
                          matterContainer.WaterMatter == 0 &&
                          matterContainer.WaterVapor == 0 &&
                          matterContainer.BioMassMatter == 0);
      if (isEmptyTerrain && emptyMatter) {
        std::cout
            << "[EffectsSystem] Empty terrain with no matter container at ("
            << pos.x << ", " << pos.y << ", " << pos.z << ")\n";
