...
public:
  explicit UnknownConfigItemError(const std::string &msg)
...
public:
  ...
  std::string get(const std::string &section, const std::string &key) const {
    ...
    if (key_it == sec.end())
      throw std::runtime_error("Key not found: " + key + " in section " +
                               section);
  ...
  bool getboolean(const std::string &section, const std::string &key) const {
    ...
    if (val == "false")
      return false;
    throw std::runtime_error(key + " must be True or False in section " +
                             section);
  }

  // Returns all key-value pairs in a section.
  std::unordered_map<std::string, std::string>
  items(const std::string &section) const {
...
template <typename T> class ConfigParameter {
public:
  std::string name;
  std::optional<T> default_value;

  // Constructor without a default value.
  explicit ConfigParameter(const std::string &name)
      : name(name), default_value(std::nullopt) {}
  ...
  T parse(const std::string &section, const ConfigParser &parser) const {
    if constexpr (std::is_same_v<T, int>) {
      ...
      throw std::runtime_error("Unexpected configuration type for key: " +
                               name);
    }
  }
  ...
  T interpret(
      const std::unordered_map<std::string, std::string> &config_dict) const {
    auto it = config_dict.find(name);
    if (it == config_dict.end()) {
      if (!default_value.has_value()) {
        throw std::runtime_error("Missing configuration item: " + name);
      } else {
        std::cerr << "Warning: Using default " << format(*default_value)
                  << " for '" << name << "'\n";
...
inline void write_pretty_params(
    std::ostream &os,
    const std::vector<std::pair<std::string, std::string>> &params) {
  size_t longest = 0;
  for (const auto &p : params) {
    if (p.first.size() > longest)
      longest = p.first.size();
  }
  ...
  Template parameters:
    - GenomeType, ReproductionType, SpeciesSetType, StagnationType:
      These types must provide:
         ...
         • A static method:
               static ConfigType parse_config(const
  std::unordered_map<std::string, std::string>& dict); • A static method: static
  void write_config(std::ostream& os, const ConfigType& config); • A static
  method or constant: section_name() that returns the section name.
*/
template <typename GenomeType, typename ReproductionType,
          typename SpeciesSetType, typename StagnationType>
class Config {
public:
  ...
  Config(const std::string &filename) {
    ...
    ConfigParameter<bool> p_no_fitness_termination("no_fitness_termination",
                                                   false);

    // Parse required parameters. For those without defaults, errors will be
    // thrown if missing.
    ...
    try {
      ...
      std::cerr
          << "Warning: Using default false for 'no_fitness_termination'\n";
    }

    // Check for unknown keys in the NEAT section.
    std::vector<std::string> known = {
        "pop_size", "fitness_criterion", "fitness_threshold",
    ...
    // Parse additional type sections using the provided types' static
    // parse_config methods.
  ...
  void save(const std::string &filename) const {
    ...
    os << "# The `NEAT` section specifies parameters particular to the NEAT "
          "algorithm\n";
    ...
    neat_params.emplace_back("fitness_threshold",
                             std::to_string(fitness_threshold));
    neat_params.emplace_back("reset_on_extinction",
...
//     static ConfigType parse_config(const std::unordered_map<std::string,
//     std::string>& dict) {
...
// };
//
// // Similar definitions must be provided for Reproduction, SpeciesSet,
// Stagnation.
//
// int main() {
//     try {
