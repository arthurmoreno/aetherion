...
DefaultGenomeConfig::DefaultGenomeConfig(const GenomeParams &params)
    : num_inputs(params.num_inputs), num_outputs(params.num_outputs),
      num_hidden(params.num_hidden), feed_forward(params.feed_forward),
      compatibility_disjoint_coefficient(
          params.compatibility_disjoint_coefficient),
  ...
  if (initial_connection.find("partial") != std::string::npos) {
    ...
    if (connection_fraction < 0.0 || connection_fraction > 1.0)
      throw std::runtime_error(
          "Partial connection fraction must be between 0.0 and 1.0.");
  }
...
int DefaultGenomeConfig::get_new_node_key(
    const std::map<int, DefaultNodeGene> &node_dict) {
  if (!node_dict.empty()) {
    auto it = std::max_element(
        node_dict.begin(), node_dict.end(),
        [](const auto &a, const auto &b) { return a.first < b.first; });
    next_node_key = it->first + 1;
  }
  // Ensure the new key is unique.
  while (node_dict.find(next_node_key) != node_dict.end())
    next_node_key++;
...
DefaultNodeGene DefaultGenome::create_node(const DefaultGenomeConfig &config,
                                           int node_key) {
  DefaultNodeGene node(node_key);
  node.init_attributes(); // Assume init_attributes() is implemented (even if
                          // empty)
  return node;
}

DefaultConnectionGene
DefaultGenome::create_connection(const std::pair<int, int> &conn_key) {
  DefaultConnectionGene conn(conn_key);
  conn.init_attributes(); // Assume init_attributes() is implemented (even if
                          // empty)
  return conn;
}
...
// ---------------------------------------------------------------------------
DefaultGenome get_pruned_copy(const DefaultGenome &genome,
