...
void imguiPrepareWorldFormWindows(nb::list &commands, nb::dict &shared_data) {
  ...
  // Form title
  ImGui::SetCursorPosX(
      (windowSize.x - ImGui::CalcTextSize("CREATE NEW WORLD").x) * 0.5f);
  ...
  ImGui::InputTextMultiline("##WorldDescription", worldDescription,
                            sizeof(worldDescription), ImVec2(0, 60));
  ...
  ImGui::SliderFloat("Evaporation Coefficient", &evaporationCoefficient, 1.0f,
                     20.0f, "%.2f");
  ImGui::SliderFloat("Heat to Water Evaporation", &heatToWaterEvaporation,
                     50.0f, 300.0f, "%.2f");
  ImGui::SliderInt("Water Minimum Units", &waterMinimumUnits, 200, 34000);

  ImGui::Spacing();
  ImGui::Text("Metabolism Settings:");
  ImGui::SliderFloat("Metabolism Cost to Apply Force",
                     &metabolismCostToApplyForce, 0.0000001f, 0.00001f, "%.8f");
  ...
  shared_data["metabolism_cost_to_apply_force"] =
      nb::float_(metabolismCostToApplyForce);
