...
void imguiPrepareCharacterListWindows(nb::list &commands,
  ...
  // Window title
  ImGui::SetCursorPosX(
      (windowSize.x - ImGui::CalcTextSize("SELECT CHARACTER").x) * 0.5f);
  ...
  ImGui::BeginChild("CharacterTableScrollRegion", ImVec2(0, -buttonAreaHeight),
                    false, ImGuiWindowFlags_None);

  // Create the character list table
  if (ImGui::BeginTable("CharacterTable", 5,
  ...
  bool hasSelection =
      selectedCharacterIndex >= 0 && !selectedCharacterKey.empty();
  if (!hasSelection) {
    ImGui::BeginDisabled();
  }
  ...
  if (hasSelection && shared_data.contains(selectedCharacterKey.c_str())) {
    auto selected_character =
        nb::cast<nb::dict>(shared_data[selectedCharacterKey.c_str()]);
