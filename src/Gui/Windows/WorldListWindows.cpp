...
void imguiPrepareWorldListWindows(nb::list &commands, nb::dict &shared_data) {
  ...
  // Window title
  ImGui::SetCursorPosX((windowSize.x - ImGui::CalcTextSize("SELECT WORLD").x) *
                       0.5f);
  ...
  ImGui::BeginChild("WorldTableScrollRegion", ImVec2(0, -buttonAreaHeight),
                    false, ImGuiWindowFlags_None);

  // Create the world list table
  if (ImGui::BeginTable("WorldTable", 4,
  ...
  if (hasSelection && shared_data.contains(selectedWorldKey.c_str())) {
    auto selected_world =
        nb::cast<nb::dict>(shared_data[selectedWorldKey.c_str()]);
  ...
  // logger->info("Can connect to world '{}': {}", selectedWorldKey,
  // canConnect);
