...
void RenderItemSlot(
  ...
  bool isEmptySlot = (current_item.texture == nullptr) ||
  ...
  if (isSelected) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Border,
                          ImVec4(1.0f, 1.0f, 0.0f, 1.0f)); // Yellow border
    ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize,
                        2.0f); // Increase border size
  }
  ...
  if (showHotkeyNumbers) {
    ...
    // Map index to hotkey number (e.g., index 0 corresponds to hotkey '1',
    // index 9 to '0')
    ...
    ImGui::GetWindowDrawList()->AddText(textPos, ImColor(255, 255, 255),
                                        hotkeyStr.c_str());
  }
  ...
  if (!isEmptySlot) {
    ...
    if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
      ...
      strncpy(payload_data.window_id, window_id.c_str(),
              sizeof(payload_data.window_id) - 1);
      payload_data.window_id[sizeof(payload_data.window_id) - 1] =
          '\0'; // Ensure null-termination

      // Set the payload with the index and window_id
      ImGui::SetDragDropPayload(payload_type.c_str(), &payload_data,
                                sizeof(DragPayload));
  ...
  if (ImGui::BeginDragDropTarget()) {
    if (const ImGuiPayload *payload =
            ImGui::AcceptDragDropPayload(payload_type.c_str())) {
      if (payload->DataSize == sizeof(DragPayload)) {
        const DragPayload *drag_payload =
            static_cast<const DragPayload *>(payload->Data);
