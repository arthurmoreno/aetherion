...
constexpr float NormalizeColor(int value) {
...
void ApplyCustomStyle(const char *fontPath) {
  ...
  ImVec4 color_BlueCyanDimmed =
      NormalizeColor(61 * 0.7f, 128 * 0.7f, 163 * 0.7f);
  ...
  style.FrameRounding =
      5.0f; // Rounding radius for frame corners (buttons, input boxes, etc.)
...
void imguiProcessEvent(nb::bytes event_bytes) {
  ...
  SDL_Event *event =
      reinterpret_cast<SDL_Event *>(const_cast<char *>(bytes.data()));
...
void RenderPhysicsSettingsWindow(nb::dict physicsChanges) {
  ...
  if (ImGui::SliderFloat("Friction Coefficient slide", &friction, 0.0f,
                         10.0f)) {
    physics->setFriction(friction);
  }
...
void RenderPlayerStatsWindow(std::shared_ptr<World> world_ptr) {
  ...
  for (auto entity : view) {
    ...
    if (entityTypeComp && entityTypeComp->mainType == 2 &&
        entityTypeComp->subType0 == 1) {
      // Retrieve additional entity components, such as position or other
      // metadata
      EntityInterface entityInterface =
          createEntityInterface(world_ptr->registry, entity);
      ...
      if (auto *physicsStats =
              world_ptr->registry.try_get<PhysicsStats>(entity)) {
        ImGui::NewLine();
...
void RenderCameraSettingsWindow(int &CAMERA_SCREEN_WIDTH_ADJUST_OFFSET,
  ...
  ImGui::SliderInt("Width Adjust Offset", &CAMERA_SCREEN_WIDTH_ADJUST_OFFSET,
                   -1000, 1000);
  ImGui::SliderInt("Height Adjust Offset", &CAMERA_SCREEN_HEIGHT_ADJUST_OFFSET,
                   -1000, 1000);
  ImGui::Separator();
  ...
  if (is_active && ImGui::IsMouseDragging(ImGuiMouseButton_Left)) {
    ...
    CAMERA_SCREEN_WIDTH_ADJUST_OFFSET =
        (mouse_pos_in_plane.x / plane_size.x) * (OFFSET_MAX - OFFSET_MIN) +
        OFFSET_MIN;

    // **Removed Y-axis inversion to align with screen coordinates (Y increases
    // downwards)**
    CAMERA_SCREEN_HEIGHT_ADJUST_OFFSET =
        (mouse_pos_in_plane.y / plane_size.y) * (OFFSET_MAX - OFFSET_MIN) +
        OFFSET_MIN;
  }

  // Calculate normalized positions for the offsets
  float normalized_x = (CAMERA_SCREEN_WIDTH_ADJUST_OFFSET - OFFSET_MIN) /
                       (OFFSET_MAX - OFFSET_MIN);
  float normalized_y = (CAMERA_SCREEN_HEIGHT_ADJUST_OFFSET - OFFSET_MIN) /
                       (OFFSET_MAX - OFFSET_MIN);

  // **Adjust the Y position without inversion**
  ImVec2 camera_pos_screen = ImVec2(p0.x + normalized_x * plane_size.x,
                                    p0.y + normalized_y * plane_size.y);
...
std::vector<InventoryItem> LoadInventory(nb::dict inventoryData) {
  ...
  for (auto item : inventoryData) {
    ...
    if (!nb::isinstance<nb::dict>(item.second)) {
      std::cerr << "Warning: Skipping non-dictionary entry in inventory data."
                << std::endl;
      continue;
    }
    ...
    if (itemDict.contains("name")) {
      try {
        ...
        std::cerr << "Error: 'name' field must be a string. Skipping item."
                  << std::endl;
        continue;
      }
    } else {
      std::cerr << "Warning: 'name' field missing. Assigning 'Unknown'."
                << std::endl;
    }
    ...
    if (itemDict.contains("texture_id")) {
      try {
        ...
        // std::cerr
        //     << "Error: 'texture_id' field must be a string. Assigning
        //     'default_texture'."
        //     << std::endl;
      ...
      std::cerr
          << "Warning: 'texture_id' field missing. Assigning 'default_texture'."
          << std::endl;
    }
    ...
    if (itemDict.contains("quantity")) {
      ...
      std::cerr << "Warning: 'quantity' field missing. Assigning 1."
                << std::endl;
    }
    ...
    if (!texture) {
      ...
      texture = getTextureFromManager(textureId);
      if (!texture) {
        std::cerr << "Error: Default texture not found. Skipping item."
                  << std::endl;
    ...
    if (ImGui::IsMouseReleased(0)) {
      ...
      if (!ImGui::GetIO().WantCaptureMouse) {
        ...
        DropToWorldCommand command(
            ...
            GuiStateManager::Instance()->src_window_id, mousePosScreen.x,
            mousePosScreen.y);
        commands.append(command);

        std::cout << "Dropped item "
                  << GuiStateManager::Instance()->draggedItemIndex << " from "
...
void RenderTopBar() {
  ...
  ImGuiWindowFlags window_flags =
      ...
      ImGuiWindowFlags_NoFocusOnAppearing | // Prevent window from taking focus
                                            // on appear
  ...
  ImGui::SetCursorPosX(ImGui::GetWindowWidth() -
                       820); // Adjusted for Console button
...
// System-level command buffer renderer - displays pending commands in the
// command queue Similar to bash's command history rendering or kernel's command
// buffer display
static void __render_command_buffer_status(const nb::list &command_queue) {
  ...
  // Limit display to prevent terminal spam (similar to dmesg ring buffer
  // behavior)
  constexpr size_t MAX_COMMANDS_DISPLAY = 5;
  const size_t display_count =
      std::min(command_queue.size(), MAX_COMMANDS_DISPLAY);
  ...
  if (command_queue.size() > display_count) {
    ImGui::Text("  ... and %zu more commands",
                command_queue.size() - display_count);
  }
...
void RenderConsoleWindow(nb::list &consoleLogs, nb::list &commands) {
  ImVec4 color_DarkBlue = NormalizeColor(30, 49, 75);
  ImVec4 semiTransparentBg =
      NormalizeColor(30, 49, 75, 0.7f); // Dark Blue with 70% opacity
  ...
  for (const auto &item : consoleLogs) {
    try {
      ...
      errorMsg += e.what();
      ImGui::TextColored(ImVec4(1.0f, 0.2f, 0.2f, 1.0f), "%s",
                         errorMsg.c_str());
    } catch (const std::exception &e) {
  ...
  // InputTextFlags_EnterReturnsTrue makes the InputText return true when Enter
  // is pressed
...
std::string get_dict_value(const nb::dict &dict, const std::string &key) {
  if (dict.contains(key.c_str())) { // Convert std::string to const char*
    ...
    std::cerr << "Warning: Key '" << key << "' not found in entity dictionary."
              << std::endl;
    return "";
  }
}
...
// const auto FEATURE_TERRAIN_GRADIENT_CAMERA_STATS =
// "terrain_gradient_camera_stats";
constexpr const char *FEATURE_WATER_CAMERA_STATS = "water_camera_stats";
constexpr const char *FEATURE_TERRAIN_GRADIENT_CAMERA_STATS =
    "terrain_gradient_camera_stats";
...
void RenderEntityInterfaceWindow(
  ...
  if (entityInterface->hasComponent(ComponentFlag::ENTITY_TYPE)) {
    EntityTypeComponent entityType =
        entityInterface->getComponent<EntityTypeComponent>();
  ...
  if (entityInterface->hasComponent(ComponentFlag::MATTER_CONTAINER)) {
    MatterContainer matterContainer =
        entityInterface->getComponent<MatterContainer>();
    ImGui::Text("WaterMatter Container Current: %d",
...
void ShowGadgetsWindow(
    nb::list &commands, nb::dict shared_data,
  ...
  bool terrainCameraStats =
      GuiStateManager::Instance()->getTerrainCameraStats();
  bool showHoveredEntitiesStats =
      GuiStateManager::Instance()->getHoveredEntityInterfaceStats();
  bool showSelectedEntitiesStats =
      GuiStateManager::Instance()->getSelectedEntityInterfaceStats();

  // Display the checkbox
  if (ImGui::Checkbox("Water Camera Stats", &waterCameraStats)) {
    GuiStateManager::Instance()->setWaterCameraStats(waterCameraStats);
    // Log the action
    std::cout << "Water Camera Stats "
  ...
  if (ImGui::Button("Hovered Entity Interface Stats")) {
    ...
    GuiStateManager::Instance()->setHoveredEntityInterfaceStats(
        showHoveredEntitiesStats);
  }

  if (ImGui::Button("Selected Entity Interface Stats")) {
    showSelectedEntitiesStats = !showSelectedEntitiesStats;
    GuiStateManager::Instance()->setSelectedEntityInterfaceStats(
        showSelectedEntitiesStats);
  }
...
void RenderAIStatisticPlot(const nb::dict &statistics,
  ...
  try {
    if (statistics.contains(plotName) &&
        nb::isinstance<nb::dict>(statistics[plotName.c_str()]))
      stat_map =
          nb::cast<std::map<std::string, double>>(statistics[plotName.c_str()]);
    else
      stat_map = nb::cast<std::map<std::string, double>>(statistics);
  } catch (const nb::cast_error &e) {
    std::cerr << "Statistics dict cast failed for plot '" << plotName
              << "': " << e.what() << '\n';
    ...
    if (statistics.contains(plotName)) {
      ...
      std::cerr << "  - Is dict: "
                << (nb::isinstance<nb::dict>(val) ? "yes" : "no") << '\n';
    }
  ...
  if (!xs.empty() && xs.size() == ys.size()) {
    ...
    ImPlot::SetNextAxisLimits(ImAxis_X1, window_left, latest,
                              ImPlotCond_Always);
    if (ImPlot::BeginPlot(plotTitle.c_str())) {
      ImPlot::SetupAxes("Timestamp", "Value");
      ImPlot::PlotBars(plotName.c_str(), xs.data(), ys.data(),
                       static_cast<int>(xs.size()), 0.67);
      ImPlot::EndPlot();
    }
  } else {
    ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f),
                       "No data available for '%s'", plotName.c_str());
    ImGui::TextWrapped("Debug: stat_map size=%zu, xs.size=%zu, ys.size=%zu",
                       stat_map.size(), xs.size(), ys.size());
  }
}
...
void RenderAIStatisticsWindow(const nb::dict &statistics) {
  ...
  RenderAIStatisticPlot(statistics, "inference_queue_size",
                        "Inference Queue Size");
  RenderAIStatisticPlot(statistics, "action_queue_size", "Action Queue Size");
  RenderAIStatisticPlot(statistics, "population_mean",
                        "Population inference interval Mean");
  RenderAIStatisticPlot(statistics, "population_max",
                        "Population inference interval Max");
  RenderAIStatisticPlot(statistics, "population_min",
                        "Population inference interval Min");
}

// Render Physics Metrics content
void RenderPhysicsMetricsWindow(const nb::dict &statistics) {
  RenderAIStatisticPlot(statistics, "physics_move_gas_entity",
                        "Move Gas Entity");
  RenderAIStatisticPlot(statistics, "physics_move_solid_entity",
                        "Move Solid Entity");
  RenderAIStatisticPlot(statistics, "physics_evaporate_water_entity",
                        "Evaporate Water Entity");
  RenderAIStatisticPlot(statistics, "physics_condense_water_entity",
                        "Condense Water Entity");
  RenderAIStatisticPlot(statistics, "physics_water_fall_entity",
                        "Water Fall Entity");
  RenderAIStatisticPlot(statistics, "physics_water_spread", "Water Spread");
  RenderAIStatisticPlot(statistics, "physics_water_gravity_flow",
                        "Water Gravity Flow");
  ...
  RenderAIStatisticPlot(statistics, "physics_vapor_merge_sideways",
                        "Vapor Merge Sideways");
  RenderAIStatisticPlot(statistics, "physics_add_vapor_to_tile_above",
                        "Add Vapor To Tile Above");
  RenderAIStatisticPlot(statistics, "physics_create_vapor_entity",
                        "Create Vapor Entity");
  RenderAIStatisticPlot(statistics, "physics_delete_or_convert_terrain",
                        "Delete or Convert Terrain");
  RenderAIStatisticPlot(statistics, "physics_invalid_terrain_found",
                        "Invalid Terrain Found");
}
...
void RenderLifeMetricsWindow(const nb::dict &statistics) {
  ...
  RenderAIStatisticPlot(statistics, "life_soft_kill_entity",
                        "Soft Kill Entity");
  RenderAIStatisticPlot(statistics, "life_hard_kill_entity",
                        "Hard Kill Entity");
  RenderAIStatisticPlot(statistics, "life_remove_velocity", "Remove Velocity");
  RenderAIStatisticPlot(statistics, "life_remove_moving_component",
...
void renderTextEditorWindow(bool *showTextEditor, nb::dict &shared_data) {
  ...
  static char textBuffer[65536] =
      ...
      "    ],\n"
      "    \"voxels_sparse\": [\n"
      "        # 8x8x8 cube centered at (8,8,8) -> x,y in [4..11], z in "
      "[4..11]\n"
      "        *[\n"
      "            {\"x\": x, \"y\": y, \"z\": z, \"color\": 1}\n"
      "            for x in range(1, 16)\n"
      ...
      "editing_sprite = Sprite(renderer, \"box_solid\", \"box_solid.png\", "
      "x=150, y=150, "
  ...
  if (ImGui::Begin("Simple Text Editor", showTextEditor,
    ...
    if (ImGui::BeginMenuBar()) {
      if (ImGui::BeginMenu("File")) {
        ...
        if (ImGui::MenuItem("Save", "Ctrl+S")) {
          ...
          // Safely add text content to shared_data using nanobind string
          // conversion
          try {
            shared_data["text_editor_content"] = nb::str(textBuffer);
          } catch (const std::exception &e) {
            std::cerr << "Error storing text editor content: " << e.what()
                      << std::endl;
          }
    ...
    if (ImGui::InputTextMultiline("##TextEditor", textBuffer,
                                  sizeof(textBuffer), textSize, flags)) {
      isDirty = true;
    }
  }
  ImGui::End();
...
std::string getTextEditorContent() {
  ...
  return "// To get actual content, the textBuffer would need to be accessible "
         "from here";
}
...
void setTextEditorContent(const std::string &content) {
  ...
  std::cout << "Setting text content: " << content.length() << " characters"
            << std::endl;
}

void showTextEditorWindow() { showTextEditor = true; }

void renderInGameGuiFrame(
    int worldTicks, float availableFps, std::shared_ptr<World> world_ptr,
    nb::dict physicsChanges, nb::dict inventoryData, nb::list &consoleLogs,
    nb::list &entitiesData, nb::list &commands, nb::dict statistics,
    nb::dict &shared_data, std::shared_ptr<EntityInterface> entityInterface_ptr,
    std::shared_ptr<EntityInterface> hoveredEntityInterface_ptr,
    std::shared_ptr<EntityInterface> selectedEntityInterface_ptr) {
  /*──────────────── Frame setup ────────────────*/
  ...
  // Ensure editor debugger is active (can be closed by user but should start
  // active)
...
void imguiPrepareEditorWindows(nb::list &commands, nb::dict &shared_data,
  ...
  ImVec2 windowPos = ImVec2((displaySize.x - windowSize.x) * 0.5f,
