 ...
 *
 * Snapshot data is stored in context.sharedData["snapshots"] as a list of
 * snapshot names. Commands are issued to take snapshots, analyze them, or
 * delete them.
 */
...
public:
  ...
  void render(GuiContext &context) override {
    ...
    if (ImGui::Begin("Editor Debugger Menu", &isActive_,
...
private:
  ...
  void renderSimulationControl(GuiContext &context) {
    if (ImGui::CollapsingHeader("Simulation Control",
      ...
      ImGui::Separator();
      // Ensure FPS value is stored in sharedData so UI stays in sync with the
      // simulation
      int fps = 60;
  ...
  void renderGameMetrics(GuiContext &context) {
    if (ImGui::CollapsingHeader("Game Metrics",
                                ImGuiTreeNodeFlags_DefaultOpen)) {
  ...
  void renderSnapshotDebugger(GuiContext &context) {
    if (ImGui::CollapsingHeader("Snapshot Debugger",
                                ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (context.sharedData.contains("snapshots")) {
        try {
          snapshots = nb::cast<std::vector<std::string>>(
              context.sharedData["snapshots"]);
        } catch (...) {
