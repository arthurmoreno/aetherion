 ...
 *
 * This function orchestrates the rendering of all GUI windows and panels during
 * active gameplay. It handles frame initialization, renders various game
 * windows (settings, inventory, stats, etc.), processes user input through the
 * console, and manages drag-drop operations between UI elements and the game
 * world.
 *
 ...
 * @param world_ptr Shared pointer to the game world (can be nullptr when world
 * is not loaded)
 * @param physicsChanges Dictionary containing physics settings that can be
 * modified via GUI
 ...
 * @param commands Output list where GUI-generated commands are appended (e.g.,
 * item transfers, settings changes)
 ...
 * @param hoveredEntityInterface_ptr Interface to the currently hovered entity
 * (if any)
 * @param selectedEntityInterface_ptr Interface to the currently selected entity
 * (if any)
 *
 * @note This function is called once per frame during gameplay mode.
 * @note The function renders windows conditionally based on user interactions
 * (toggle flags).
 */
...
void imguiPrepareEditorRuntimeDebuggerWindows(nb::list &commands,
                                              nb::dict &shared_data);
 ...
 *
 * Displays a collapsible, draggable toolbar with editor controls for Play,
 * Stop, Step, Exit to Editor, and Settings. This toolbar is used during editor
 * runtime debugging mode.
 *
 * @param commands Output list where editor commands are appended (e.g., Play,
 * Stop, Step)
 */
void RenderEditorDebuggerTopBar(nb::list &commands);

void imguiPrepareWorldTypeFormWindows(nb::list &commands,
                                      nb::dict &shared_data);

void imguiPrepareServerWorldFormWindows(nb::list &commands,
                                        nb::dict &shared_data);
...
void imguiPrepareCharacterFormWindows(nb::list &commands,
                                      nb::dict &shared_data);

void imguiPrepareCharacterListWindows(nb::list &commands,
                                      nb::dict &shared_data);
