...
void QueueCommand::execute(GuiContext& context, std::deque<TerminalLine>& terminalBuffer,
                           bool& scrollToBottom) {
    ...
    addOutput(terminalBuffer, "Pending commands: " + std::to_string(command_queue.size()), false,
              false);
    ...
    if (command_queue.size() > display_count) {
        std::string more =
            "  ... and " + std::to_string(command_queue.size() - display_count) + " more commands";
