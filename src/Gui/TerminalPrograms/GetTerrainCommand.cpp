...
void GetTerrainCommand::execute(GuiContext &context,
                                std::deque<TerminalLine> &terminalBuffer,
                                bool &scrollToBottom) {
  if (tokens_.size() < 3) {
    addOutput(terminalBuffer,
              "Usage: get_terrain <x> <y> <z>  (e.g. get_terrain 5 50 8)",
              false, true);
    scrollToBottom = true;
    return;
  }
  ...
  nb::dict command;
  command[nb::str("type")] = nb::str("query_get_terrain");
  command[nb::str("params")] = params;

  context.commands.append(command);

  addOutput(terminalBuffer,
            "Querying terrain at (" + std::to_string(x) + ", " +
