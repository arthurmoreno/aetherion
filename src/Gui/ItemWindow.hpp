...
struct InventoryItem {
  ...
  InventoryItem(const std::string &itemName, const std::string &texId,
                SDL_Texture *tex, int qty)
...
void RenderItemSlot(
    std::vector<InventoryItem> &items, int index, int &selected_index,
    const ImVec2 &buttonSize, const std::string &button_prefix,
    const std::string &payload_type, const std::string &window_id,
    nb::list &commands,
    const std::function<void(int, InventoryItem &)> &customLogic,
...
public:
  ...
  virtual void
  Render() = 0; // Pure virtual function to be implemented by derived classes
};
  ...
  void Render() override {
    ...
    if (ImGui::BeginTable("InventoryTable", columns,
      ...
      for (size_t i = 0; i < items->size(); ++i) {
        ...
        RenderItemSlot(*items, i, selected_index, buttonSize, button_prefix,
                       payload_type, "inventory", *commands, nullptr, false);
      }
      ImGui::EndTable();
    }
...
public:
  ...
  void Render() override {
    ...
    size_t hotbar_size =
        std::min(items->size(), static_cast<size_t>(HOTBAR_SIZE));
    ...
    ImVec2 window_pos = ImVec2(ImGui::GetIO().DisplaySize.x * 0.5f,
    ...
    for (size_t i = 0; i < hotbar_size; ++i) {
      ...
      // Render the item slot
      RenderItemSlot(*items, i, selected_index, buttonSize, button_prefix,
                     payload_type, "hotbar", *commands, nullptr, true);
...
public:
  ...
  void Render() override {
    ...
    RenderItemSlot(*items, slotIndices["head"], selected_index, ImVec2(32, 32),
                   "EquipHead_", "DND_ITEM", "equipment", *commands, nullptr,
                   false);

    // Render chest slot
    ImGui::SetCursorPos(chestSlotPos);
    RenderItemSlot(*items, slotIndices["chest"], selected_index, ImVec2(32, 32),
                   "EquipChest_", "DND_ITEM", "equipment", *commands, nullptr,
                   false);
