...
public:
   ...
   */
  bool executeCommandInTerminal(const std::string &programId,
                                const std::string &command,
                                GuiContext &context) {
    auto program = getProgram(programId);
    if (!program) {
      std::cerr << "[TerminalProgramManager] Warning: Terminal program '"
                << programId << "' not found for command execution."
                << std::endl;
      return false;
    }
    ...
    std::cerr << "[TerminalProgramManager] Command routing not yet implemented."
              << std::endl;
    return true;
  }
   ...
   */
  std::vector<std::string> getActiveTerminalIds() const {
