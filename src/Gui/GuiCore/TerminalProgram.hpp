 ...
 * - Custom commands can be implemented by derived classes
...
public:
  ...
  virtual void render(GuiContext &context) {
    ...
    // Begin the terminal window with display name
    std::string windowTitle = getDisplayName();
    ImGui::Begin(windowTitle.c_str(), &isActive_,
    ...
    // Auto-scroll to the bottom if scrollToBottom is true
    if (scrollToBottom_) {
...
protected:
  ...
  virtual void clearTerminal() {
    ...
    addOutput("Terminal cleared. Type 'help' for available commands.", false,
              false);
  ...
  virtual void showHelp() {
    ...
    addOutput("Command format: <type> param1=value1 param2=value2 ...", false,
              false);
    addOutput("Navigation: Use Up/Down arrows to navigate history", false,
              false);
    scrollToBottom_ = true;
  }
   ...
   */
  virtual void showCommandQueue(const nb::list &command_queue) {
    ...
    addOutput("Pending commands: " + std::to_string(command_queue.size()),
              false, false);
    ...
    if (command_queue.size() > display_count) {
      std::string more = "  ... and " +
                         std::to_string(command_queue.size() - display_count) +
                         " more commands";
   ...
   */
  virtual bool handleCustomCommand(const std::string &commandStr,
                                   GuiContext &context) {
...
private:
  ...
  static int TextEditCallbackStub(ImGuiInputTextCallbackData *data) {
    TerminalProgram *terminal = (TerminalProgram *)data->UserData;
  ...
  void executeCommand(GuiContext &context) {
    ...
    // Try custom command handling first
    if (handleCustomCommand(commandStr, context)) {
      scrollToBottom_ = true;
      return;
    }

    // Parse and execute custom command (default behavior)
    try {
