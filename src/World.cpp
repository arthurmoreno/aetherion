  ...
  for (const auto &[entity, _] : lifeEngine->entitiesToRemoveMovingComponent) {
    ...
    if (hasMoving) {
      ...
      console->debug("[Moving] Skipped entity {} (valid={}, hasMoving={})",
                     entityId, valid, hasMoving);
      movingSkipped++;
    }
  }
  console->debug("[Moving Summary] Removed: {}, Skipped: {}", movingRemoved,
                 movingSkipped);
    ...
    if (!isSpecialId && isValidEntity) {
      ...
      if (registry.all_of<Position, EntityTypeComponent>(entity)) {
        ...
        console->debug("[Deletion] Entity {} position: ({},{},{}), type: {}/{}",
                       entityId, pos.x, pos.y, pos.z, type.mainType,
                       type.subType0);
        ...
        console->debug(
            "[Deletion] Entity {} lacks Position/EntityTypeComponent",
            entityId);
      }
      ...
      try {
        ...
        console->debug(
            "[Deletion] Before erase - entitiesScheduledForDeletion size: {}",
      ...
      if (isSpecialId) {
        ...
        try {
          ...
          console->debug("[Deletion] Before erase (invalid) - "
                         "entitiesScheduledForDeletion size: {}",
                         lifeEngine->entitiesScheduledForDeletion.size());
          size_t erased =
              lifeEngine->entitiesScheduledForDeletion.erase(entity);
          console->debug("[Deletion] Erased {} entities from scheduled "
                         "deletion (invalid). New "
                         "size: {}",
                         erased,
                         lifeEngine->entitiesScheduledForDeletion.size());
          console->debug("[Deletion] Force-destroyed invalid entity {}",
                         entityId);
          successfulDeletions++;
        } catch (const std::exception &e) {
          console->error(
              "[Deletion] EXCEPTION while force-destroying entity {}: {}",
              entityId, e.what());
          skippedDeletions++;
