...
// Prefer std::vector<uint8_t> to leverage Embind's automatic conversions from
// TypedArray/Array.
static EntityInterface
EntityInterface_deserialize_vec(const std::vector<unsigned char> &bytes) {
  if (bytes.empty()) {
    throw std::runtime_error("EntityInterface.deserialize: empty buffer");
  }
  try {
    return EntityInterface::deserialize(
    ...
    emscripten::val::global("console").call<void>(
        "error",
        std::string("[embind] deserialize(header) failed: ") + ex.what(),
        std::string(" len=") + std::to_string(bytes.size()),
        std::string(" head=") +
            to_hex(bytes.data(), std::min<size_t>(24, bytes.size())));
    // Try an offset=4 diagnostic (struct_pack version prefix?) and log whether
    // it works
    try {
      ...
      emscripten::val::global("console").call<void>(
          "warn",
          std::string(
              "[embind] header parsed with offset=4 (diagnostic only)"));
    } catch (...) {
      // ignore
    }
    throw; // rethrow
...
// Fallback: accept arbitrary JS object (Buffer, ArrayBuffer, Uint8Array) when
// vector overload is not picked.
static EntityInterface EntityInterface_deserialize_js(emscripten::val any) {
  try {
    ...
    if (any.instanceof (emscripten::val::global("Uint8Array"))) {
      ...
      // Respect byteOffset and byteLength when present (Node.js Buffer,
      // DataView, etc.)
      size_t byteOffset =
      ...
      throw std::runtime_error(
          "EntityInterface.deserialize: unsupported input type");
    }
...
EntityInterface_serialize_js(const EntityInterface &e) {
...
static val EntityInterface_get_moving_component(const EntityInterface &e) {
  if (!e.hasComponent(MOVING_COMPONENT))
    return val::undefined();
...
// Expose MatterContainer as a plain JS object with snake_case keys (Python
// parity)
static val EntityInterface_get_matter_container(const EntityInterface &e) {
  if (!e.hasComponent(MATTER_CONTAINER))
    return val::undefined();
...
static void EntityInterface_set_physics_stats_js(EntityInterface &e, val obj) {
  ...
  auto getf = [&](const char *a, const char *b) {
    if (obj.hasOwnProperty(a))
      return obj[a].as<float>();
    if (obj.hasOwnProperty(b))
      return obj[b].as<float>();
    return 0.0f;
  };
...
static void EntityInterface_set_health_js(EntityInterface &e, val obj) {
  HealthComponent h{};
  h.healthLevel =
      obj.hasOwnProperty("health_level")  ? obj["health_level"].as<float>()
...
static void EntityInterface_set_perception_js(EntityInterface &e, val obj) {
  ...
  pc.perception_area =
      obj.hasOwnProperty("perception_area")  ? obj["perception_area"].as<int>()
      : obj.hasOwnProperty("perceptionArea") ? obj["perceptionArea"].as<int>()
                                             : 0;
  pc.z_perception_area = obj.hasOwnProperty("z_perception_area")
                             ? obj["z_perception_area"].as<int>()
                         : obj.hasOwnProperty("zPerceptionArea")
                             ? obj["zPerceptionArea"].as<int>()
                             : 0;
...
static void EntityInterface_set_metabolism_js(EntityInterface &e, val obj) {
  ...
  m.energyReserve =
      obj.hasOwnProperty("energy_reserve")  ? obj["energy_reserve"].as<float>()
      : obj.hasOwnProperty("energyReserve") ? obj["energyReserve"].as<float>()
                                            : 0.0f;
  m.maxEnergyReserve = obj.hasOwnProperty("max_energy_reserve")
                           ? obj["max_energy_reserve"].as<float>()
                       : obj.hasOwnProperty("maxEnergyReserve")
                           ? obj["maxEnergyReserve"].as<float>()
                           : 0.0f;
...
public:
  ...
  PerceptionResponseFlatBStub(const std::vector<unsigned char> &bytes)
      : ticks_(0) {
    world_ = std::make_unique<WorldViewStub>(64, 36, 8);
  ...
  EntityInterface *get_item_from_inventory_by_id(int /*id*/) const {
    return nullptr;
  }
  std::vector<unsigned char> get_query_response_by_id(int /*id*/) const {
    return {};
  }
...
EMSCRIPTEN_BINDINGS(aetherion_bindings) {
  ...
  class_<EntityInterface>("EntityInterface")
      ...
      // Use robust JS-aware converter that handles
      // Uint8Array/ArrayBuffer/Buffer correctly
  ...
  class_<PerceptionResponseFlatBStub>("PerceptionResponseFlatB")
      ...
      .function("getWorldView", &PerceptionResponseFlatBStub::getWorldView,
                allow_raw_pointers())
      .function("getEntity", &PerceptionResponseFlatBStub::getEntity,
                allow_raw_pointers())
      .function("get_item_from_inventory_by_id",
                &PerceptionResponseFlatBStub::get_item_from_inventory_by_id,
                allow_raw_pointers())
